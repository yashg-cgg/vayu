   ...
   public class CRRServer : ISourceSink
    {
        ...
        private SqlCommand sSelectSourceAndSink6MonthCRRCommand;
        /// <summary>
        ...
        private void InitDB()
        {
            ...
            //sSelectSourceAndSink6MonthCRRCommand
            sSelectSourceAndSink6MonthCRRCommand = new SqlCommand();
            sSelectSourceAndSink6MonthCRRCommand.CommandText = "select SourceName, SinkName, TimeUse, 'Annual', Bid, Hedge, sum(MW) as clearedmw, "+
           "sum(MW * ShadowPrice) / sum(MW) as obligationmcp, sum(MW * ShadowPrice) / sum(MW) as optionmcp, 'Varient', min(CRR_ID), "+ 
        "AccountHolder, Sourcekey, [Sinkkey], C.Zone as SourceZone, D.Zone as SinkZone, B.CRRAuctionName, periodKey, B.CRRAuctionKey,(ShadowPrice)ShadowPrice "+
          "from Vayu..CRRAnnualAuctionResult A left join Node C on A.Sourcekey = C.NodeKey left "+
           "join Node D on A.[Sinkkey] = D.NodeKey, Vayu..CRRAuction B where A.CRRAuctionKey = B.CRRAuctionKey "+
        "and A.AccountHolder in (@participant) and A.StartDate = @StartDate and A.CRRAuctionKey in (select CRRAuctionKey from crrauction where  CRRAuctionName like '%@Year%' and CRRAuctionType='Annual' and AuctionRound in (@Round)) " +
        "group by SourceName, SinkName, TimeUse, periodkey, Bid, Hedge, AccountHolder, Sourcekey, [Sinkkey], C.Zone, D.Zone, "+
        "B.CRRAuctionName, periodkey, B.CRRAuctionKey,ShadowPrice order by SourceName, SinkName";
          //  sSelectSourceAndSink6MonthCRRCommand.Parameters.AddWithValue("@StartDate", "StartDate");
            sSelectSourceAndSink6MonthCRRCommand.Parameters.AddWithValue("@AuctionKey", "AuctionKey");
           // sSelectSourceAndSink6MonthCRRCommand.Parameters.AddWithValue("@Round", "Round");
            sSelectSourceAndSink6MonthCRRCommand.Parameters.AddWithValue("@Year", "Year");
            sSelectSourceAndSink6MonthCRRCommand.CommandTimeout = 300;
            sSelectSourceAndSink6MonthCRRCommand.Connection = VayuDBConnection;
        ...
        private List<SourceSink> Get6MonthSourceSinkList(int marketKey, string account, DateTime period, List<long> nodeList, string round)
        {
            InitDB();
            List<SourceSink> sourceSinkList = new List<SourceSink>();
            ...
            if(VayuDBConnection.State == ConnectionState.Open)
            {
                ...
                "B.CRRAuctionName, periodkey, B.CRRAuctionKey,ShadowPrice order by SourceName, SinkName";
                    ...
                    sSelectSourceAndSink6MonthCRRCommand.CommandText = sSelectSourceAndSink6MonthCRRCommand.CommandText.Replace("@StartDate", firstDay.ToString());
                    sSelectSourceAndSink6MonthCRRCommand.CommandText = sSelectSourceAndSink6MonthCRRCommand.CommandText.Replace("@Year", firstDay.Year.ToString());
                    sSelectSourceAndSink6MonthCRRCommand.CommandText = sSelectSourceAndSink6MonthCRRCommand.CommandText.Replace("@participant", account);
                        sSelectSourceAndSink6MonthCRRCommand.CommandText = sSelectSourceAndSink6MonthCRRCommand.CommandText.Replace("@Round", round);
                        reader = sSelectSourceAndSink6MonthCRRCommand.ExecuteReader();
