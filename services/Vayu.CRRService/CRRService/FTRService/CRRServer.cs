...
namespace Vayu.CRRService
{
    ...
    class CRRServer : ISourceSink
    {
        ...
        /// <summary>
        /// The select all ercot source and sink CRR command
        /// </summary>
        private SqlCommand mSelectAllErcotSourceAndSinkCRRCommand;
        /// <summary>
        /// The select all miso source and sink CRR command
        /// </summary>
        private SqlCommand mSelectAllMisoSourceAndSinkCRRCommand;
        /// <summary>
        /// The select all caiso source and sink CRR command
        /// </summary>
        private SqlCommand mSelectAllCaisoSourceAndSinkCRRCommand;
        /// <summary>
        /// The select all SPP source and sink CRR command
        /// </summary>
        private SqlCommand mSelectAllSPPSourceAndSinkCRRCommand;
        /// <summary>
        /// The select all PJM source and sink CRR command
        /// </summary>
        private SqlCommand mSelectAllPJMSourceAndSinkCRRCommand;
        /// <summary>
        ...
        Dictionary<int, Dictionary<int, MonthlyValues>> yearHash1 = new Dictionary<int, Dictionary<int, MonthlyValues>>();
        ...
        public List<SourceSink> Get6MonthCRRs(int marketKey, List<string> accounts, DateTime period, string auctRound)
        {
            ...
            if (marketKey == 9)
            {
                ...
                try
                {
                    ...
                    //sourceSinkList = Get6MonthSourceSinkList(marketKey, account, period, nodeList, auctRound);
        ...
        public void Connect()
        {
            using (ServiceHost host = new ServiceHost(typeof(CRRServer), new Uri("net.tcp://localhost:8006")))
            {
                ...
                {
                    host.Open();
                    Console.WriteLine("Successfully opened port 8006.");
                    Console.ReadLine();
                    host.Close();
                }
                catch (Exception e)
        ...
        private void InitDB()
        {
            ...
            // mSelectCalendarCommand.CommandText = "SELECT MarketDateTime, PeakYN as flag FROM  MarketTime where MarketYear = @marketyear";

            mSelectCalendarCommand.CommandText = "SELECT MarketDateTime, PeakYN as flag  FROM  MarketTime where PeakYN = 'w' and DATENAME(DW, MarketDateTime) not in ('Saturday', 'Sunday') order by MarketDateTime";
            ...
            //
            //mSelectAllErcotSourceAndSinkCRRCommand = new SqlCommand();
            //mSelectAllErcotSourceAndSinkCRRCommand.CommandText = "select 0, A.AccountHolder, A.Category, A.HedgeType, A.Class, A.Source, B.NodeKey " +
            ...
            //mSelectAllErcotSourceAndSinkCRRCommand.Parameters.AddWithValue("@StartDate", "StartDate");
            //mSelectAllErcotSourceAndSinkCRRCommand.Parameters.AddWithValue("@EndDate", "EndDate");
            //mSelectAllErcotSourceAndSinkCRRCommand.CommandTimeout = 300;
            //mSelectAllErcotSourceAndSinkCRRCommand.Connection = VayuDBConnection;
            //


            mSelectAllErcotSourceAndSinkCRRCommand = new SqlCommand();
            mSelectAllErcotSourceAndSinkCRRCommand.CommandText = "select SourceName, SinkName, TimeUse, 'Monthly', Bid, Hedge, sum(MW) as clearedmw," +
                                                                ...
                                                                "from  CRRAuctionResults A left join Node C on A.Sourcekey = C.NodeKey " +
                                                                "left join Node D on A.[Sinkkey] = D.NodeKey,  CRRAuction B " +
                                                                ...
                                                                "and A.CRRAuctionKey in (select CRRAuctionKey from  CRRAuction where CRRAuctionStartDate between @startdate and @enddate) " +
            ...
            mSelectAllCaisoSourceAndSinkCRRCommand = new SqlCommand();
            mSelectAllCaisoSourceAndSinkCRRCommand.CommandText = "select sourcenode, sinknode, classtype, periodtype, tradetype, hedgetype, sum(clearedmw) as clearedmw, " +
                                                                "sum(clearedmw*clearingprice)/sum(clearedmw) as obligationmcp, sum(clearedmw*clearingprice)/sum(clearedmw) as optionmcp, 'Varient', min(CRRid), " +
                                                                "participant, sourcenodekey, sinknodekey, C.Zone as SourceZone, D.Zone as SinkZone, B.CRRAuctionName, periodKey, B.CRRauctionkey " +
                                                                "from caiso.CRRAuctionResults A left join Node C on A.sourcenodekey = C.NodeKey " +
                                                                "left join Node D on A.sinknodekey = D.NodeKey, caiso.CRRauction B " +
                                                                "where A.CRRauctionkey = B.CRRauctionkey and A.participant in (@participant) " +
                                                                "and A.CRRAuctionKey in (select CRRAuctionKey from caiso.CRRAuction where CRRAuctionStartDate between @startdate and @enddate) " +
                                                                ...
                                                                "B.CRRAuctionName, periodkey, B.CRRAuctionKey order by sourcenode, sinknode";
            mSelectAllCaisoSourceAndSinkCRRCommand.Parameters.AddWithValue("@startdate", "startdate");
            mSelectAllCaisoSourceAndSinkCRRCommand.Parameters.AddWithValue("@enddate", "enddate");
            mSelectAllCaisoSourceAndSinkCRRCommand.CommandTimeout = 300;
            mSelectAllCaisoSourceAndSinkCRRCommand.Connection = VayuDBConnection;
            //
            mSelectAllMisoSourceAndSinkCRRCommand = new SqlCommand();
            mSelectAllMisoSourceAndSinkCRRCommand.CommandText = "select sourcenode, sinknode, classtype, periodtype, tradetype, hedgetype, sum(clearedmw) as clearedmw, " +
                                                                "sum(clearedmw*clearingprice)/sum(clearedmw) as obligationmcp, sum(clearedmw*clearingprice)/sum(clearedmw) as optionmcp, 'Varient', min(CRRid), " +
                                                                "participant, sourcenodekey, sinknodekey, C.Zone as SourceZone, D.Zone as SinkZone, B.CRRAuctionName, periodKey, B.CRRauctionkey " +
                                                                "from MISO.CRRAuctionResults A left join Node C on A.sourcenodekey = C.NodeKey " +
                                                                "left join Node D on A.sinknodekey = D.NodeKey, miso.CRRauction B " +
                                                                "where A.CRRauctionkey = B.CRRauctionkey and A.participant in (@participant) " +
                                                                "and A.CRRAuctionKey in (select CRRAuctionKey from MISO.CRRAuction where CRRAuctionStartDate between @startdate and @enddate) " +
                                                                "group by sourcenode, sinknode, classtype, periodtype, tradetype, hedgetype, participant, sourcenodekey, sinknodekey, C.Zone, D.Zone," +
                                                                "B.CRRAuctionName, periodkey, B.CRRAuctionKey order by sourcenode, sinknode";
            mSelectAllMisoSourceAndSinkCRRCommand.Parameters.AddWithValue("@startdate", "startdate");
            mSelectAllMisoSourceAndSinkCRRCommand.Parameters.AddWithValue("@enddate", "enddate");
            mSelectAllMisoSourceAndSinkCRRCommand.CommandTimeout = 300;
            mSelectAllMisoSourceAndSinkCRRCommand.Connection = VayuDBConnection;
            //
            mSelectAllSPPSourceAndSinkCRRCommand = new SqlCommand();
            mSelectAllSPPSourceAndSinkCRRCommand.CommandText = "select sourcenode, sinknode, classtype, periodtype, tradetype, hedgetype, sum(clearedmw) as clearedmw, " +
                                                                "sum(clearedmw*clearingprice)/sum(clearedmw) as obligationmcp, sum(clearedmw*clearingprice)/sum(clearedmw) as optionmcp, 'Varient', min(CRRid), " +
                                                                "participant, sourcenodekey, sinknodekey, C.Zone as SourceZone, D.Zone as SinkZone, B.CRRAuctionName, periodKey, B.CRRauctionkey " +
                                                                "from SPP.CRRAuctionResults A left join Node C on A.sourcenodekey = C.NodeKey " +
                                                                "left join Node D on A.sinknodekey = D.NodeKey, spp.CRRauction B " +
                                                                "where A.CRRauctionkey = B.CRRauctionkey and A.participant in (@participant) " +
                                                                "and A.CRRAuctionKey in (select CRRAuctionKey from SPP.CRRAuction where CRRAuctionStartDate between @startdate and @enddate) " +
                                                                "group by sourcenode, sinknode, classtype, periodtype, tradetype, hedgetype, participant, sourcenodekey, sinknodekey, C.Zone, D.Zone, " +
                                                                "B.CRRAuctionName, periodkey, B.CRRAuctionKey order by sourcenode, sinknode";
            mSelectAllSPPSourceAndSinkCRRCommand.Parameters.AddWithValue("@startdate", "startdate");
            mSelectAllSPPSourceAndSinkCRRCommand.Parameters.AddWithValue("@enddate", "enddate");
            mSelectAllSPPSourceAndSinkCRRCommand.CommandTimeout = 300;
            mSelectAllSPPSourceAndSinkCRRCommand.Connection = VayuDBConnection;
            //
            mSelectAllPJMSourceAndSinkCRRCommand = new SqlCommand();
            mSelectAllPJMSourceAndSinkCRRCommand.CommandText = "select sourcenodename, sinknodename, classtype, periodtype, tradetype, hedgetype, sum(clearedmw) as clearedmw, sum(clearedmw*obligationPrice)/sum(clearedmw) " +
                                                                "as obligationmcp, sum(clearedmw*optionprice)/sum(clearedmw) as optionmcp, 'Varient', min(CRRid), participant, sourcenodekey, sinknodekey, N.Zone as SourceZone, M.Zone " +
                                                                "as SinkZone, min(b.CRRAuctionName), periodkey, b.CRRauctionkey from pjm.CRRauctionresults a left join (select distinct(ExternalNodeID), MIN(NodeKey) as NodeKey, MIN(Zone) as Zone " +
                                                                ...
                                                                "MIN(Zone) as Zone from Node group by  ExternalNodeID) M on a.sinknodekey = M.ExternalNodeID,  pjm.CRRauction b " +
                                                                "where a.CRRauctionkey = b.CRRauctionkey and A.participant in (@participant) " +
                                                                " and a.CRRAuctionKey in (select CRRAuctionKey from PJM.CRRAuction where CRRAuctionStartDate <= @enddate and CRRAuctionEndDate >= @startdate) " +
                                                                ...
                                                                "b.CRRauctionkey, periodkey order by sourcenodename, sinknodename";
            mSelectAllPJMSourceAndSinkCRRCommand.Parameters.AddWithValue("@startdate", "startdate");
            mSelectAllPJMSourceAndSinkCRRCommand.Parameters.AddWithValue("@enddate", "enddate");
            mSelectAllPJMSourceAndSinkCRRCommand.CommandTimeout = 300;
            mSelectAllPJMSourceAndSinkCRRCommand.Connection = VayuDBConnection;
            //
            ...
            mSelectPriceCommandPJMOptionMonthly.CommandText = "select c.StartDate,   a.LMPOnPeak, a.LMPOffPeak , c.PeakHrs, c.OffPeakHrs, c.periodkey "
                                + "from pjm.CRRAuctionOptionPrice a join period c "
        ...
        private void SetPeriodDateHash(int marketKey, DateTime period)
        {
            ...
            if (!sPeriodDateHash.ContainsKey(key))
            {
                ...
                if (marketKey == 9)
                    mSelectPeriodDateCommand.CommandText = "select peakhrs, offpeakhrs, 24hrs,PeakWE from period where startdate = @startdate and enddate = @enddate and marketkey = @marketkey";
                ...
                while (reader.Read())
                {
                    CRRHours CRRHours = new CRRHours();
                    CRRHours.Peak = reader.GetInt32(0);
                    CRRHours.OffPeak = reader.GetInt32(1);
                    CRRHours.Total = reader.GetInt32(2);
                    if (marketKey == 9)
                        CRRHours.PeakWE = reader.GetInt32(3);
                    sPeriodDateHash.Add(key, CRRHours);
                    break;
                }
                reader.Close();
                VayuDBConnection.Close();
        ...
        private void DownloadDAPrice(DateTime period, int marketKey, List<long> nodeList)
        {
            ...
            DARTNode.GetDADART(rtList, daList, firstDay, days, false, false, true);
        }
        /// <summary>
        ...
        private void Calculation(int marketKey, DateTime period, List<SourceSink> sourceSinkList)
        {
            ...
            foreach (SourceSink sourceSink in sourceSinkList)
            {
                ...
                CRRHours CRRPeriodKeyHours = null;
                ...
                if (marketKey == 2 || marketKey == 7 || marketKey == 12 || marketKey == 1 || marketKey == 9)
                {
                    ...
                    if (!sPeriodDateHash.ContainsKey(key))
                    {
                        ...
                        CRRPeriodKeyHours = sPeriodDateHash[key];
                }
                if (marketKey == 9)
                {
                ...
                if (sourceSink.ClassType.ToUpper() == "ONPEAK" || sourceSink.ClassType.ToUpper() == "PEAKWD" || sourceSink.ClassType.ToUpper() == "PEAKWE")
                {
                    if (marketKey == 9)
                    {
                        ...
                        if (sourceSink.ClassType.ToUpper() == "ONPEAK" || sourceSink.ClassType.ToUpper() == "PEAKWD")
                            totalCost = (costMw * sourceSink.MW) * CRRPeriodKeyHours.Peak;
                        else
                            totalCost = (costMw * sourceSink.MW) * CRRPeriodKeyHours.PeakWE;
                    }
                    else
                    {
                        costMw = CalculateCost(marketKey, sourceSink, period);
                        if (CRRPeriodKeyHours != null)
                        {
                            cost = costMw / (double)CRRPeriodKeyHours.Peak;
                        }
                        else
                        {
                ...
                else //ofpeaklist
                {
                    if (marketKey == 9)
                    {
                        ...
                        costMw = CalculateCost(marketKey, sourceSink, period);
                        cost = costMw;
                        totalCost = (costMw * sourceSink.MW) * CRRPeriodKeyHours.OffPeak;
                    }
                    else
                    {
                        costMw = CalculateCost(marketKey, sourceSink, period);
                        if (CRRPeriodKeyHours != null)
                        {
                            cost = costMw / (double)CRRPeriodKeyHours.OffPeak;
                        }
                        else
                        {
        ...
        private double CalculateCost(int marketKey, SourceSink sourceSink, DateTime period)
        {
            ...
            {
                string key = marketKey + period.ToShortDateString();
                CRRHours CRRPeriodHours = sPeriodDateHash[key];
                CRRHours CRRPeriodKeyHours = sPeriodHourHash[sourceSink.PeriodKey];
                if (sourceSink.ClassType.ToUpper() == "ONPEAK" || sourceSink.ClassType.ToUpper() == "PEAKWD")
                {
                    newCost = cost * ((double)CRRPeriodHours.Peak / (double)CRRPeriodKeyHours.Peak);
                }
                else if (sourceSink.ClassType.ToUpper() == "24H")
                {
                    newCost = cost * (((double)CRRPeriodHours.Peak + (double)CRRPeriodHours.OffPeak) / ((double)CRRPeriodKeyHours.Peak + (double)CRRPeriodKeyHours.OffPeak));
                }
                ...
                {
                    newCost = cost * ((double)CRRPeriodHours.OffPeak / (double)CRRPeriodKeyHours.OffPeak);
                }
                return newCost;
            }
            return cost;
        ...
        private List<SourceSink> GetSourceSinkList(int marketKey, string account, DateTime period, List<long> nodeList)
        {
            ...
            //    int CRRMonth = period.Month;
            //    string CRRMonthString = period.ToString("M").ToUpper().Substring(0, 3);
            ...
            //            DateTime firstDay = new DateTime(period.Year, period.Month, 1);
            //            DateTime lastDay = firstDay.AddMonths(1);
            //            mSelectAllErcotSourceAndSinkCRRCommand.Parameters["@StartDate"].Value = firstDay;
            //            mSelectAllErcotSourceAndSinkCRRCommand.Parameters["@EndDate"].Value = lastDay;
            //            mSelectAllErcotSourceAndSinkCRRCommand.CommandText = mSelectAllErcotSourceAndSinkCRRCommand.CommandText.Replace("@participant", account);
            //            reader = mSelectAllErcotSourceAndSinkCRRCommand.ExecuteReader();
            ...
            //                //sourcesink.CRRid = (long)(reader.GetValue(10));
            ...
            //                        CRRHours CRRHours = new CRRHours();
            //                        CRRHours.Peak = reader1.GetInt32(2);
            //                        CRRHours.OffPeak = reader1.GetInt32(3);
            //                        CRRHours.Total = reader1.GetInt32(4);
            //                        sPeriodHourHash.Add(sourcesink.PeriodKey, CRRHours);
            //                        while (startDate < endDate)
            //                        {
            ...
            if (marketKey == 9)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    if (marketKey == 9)
                    {
                        ...
                        using (SqlDataReader reader = mSelectAllErcotSourceAndSinkCRRCommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                SourceSink sourcesink = new SourceSink();
                                ...
                                sourcesink.TradeType = reader.GetString(4).Trim();
                                sourcesink.HedgeType = reader.GetString(5).Trim();
                                sourcesink.MW = (sourcesink.TradeType.ToUpper() == "SELL") ? -((double)reader.GetDecimal(6)) : (double)reader.GetDecimal(6);
                                sourcesink.Obligation = reader.IsDBNull(7) ? 0 : (double)reader.GetDecimal(7);
                                sourcesink.Option = (!reader.IsDBNull(8)) ? (double)reader.GetDecimal(8) : 0;
                                sourcesink.Period = reader.GetString(9).Trim();
                                sourcesink.Ftrid = Convert.ToInt64(reader.GetValue(10));
                                ...
                                if (!sPeriodHash.ContainsKey(sourcesink.PeriodKey))
                                {
                                    ...
                                    using (SqlDataReader reader1 = mSelectPeriodCommand.ExecuteReader())
                                    {
                                        while (reader1.Read())
                                        {
                                            List<DateTime> monthList = new List<DateTime>();
            ...
            {
                int CRRYear = (period < DateTime.Parse("6/1/" + period.Year)) ? period.Year - 1 : period.Year;
                if (marketKey == 7)
                {
                    CRRYear = period.Year;
                }
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        //DateTime firstDay = new DateTime(period.Year, period.Month, 1);
                        mSelectAllPJMSourceAndSinkCRRCommand.Parameters["@startdate"].Value = DateTime.Parse("6/1/" + CRRYear);
                        mSelectAllPJMSourceAndSinkCRRCommand.Parameters["@enddate"].Value = DateTime.Parse("May/31/" + (CRRYear + 1));
                        mSelectAllPJMSourceAndSinkCRRCommand.CommandText = mSelectAllPJMSourceAndSinkCRRCommand.CommandText.Replace("@participant", account);
                        reader = mSelectAllPJMSourceAndSinkCRRCommand.ExecuteReader();
                    }
                    else if (marketKey == 2)
                    {
                        mSelectAllMisoSourceAndSinkCRRCommand.Parameters["@startdate"].Value = DateTime.Parse("6/1/" + CRRYear);
                        mSelectAllMisoSourceAndSinkCRRCommand.Parameters["@enddate"].Value = DateTime.Parse("5/31/" + (CRRYear + 1));
                        mSelectAllMisoSourceAndSinkCRRCommand.CommandText = mSelectAllMisoSourceAndSinkCRRCommand.CommandText.Replace("@participant", account);
                        reader = mSelectAllMisoSourceAndSinkCRRCommand.ExecuteReader();
                    }
                    else if (marketKey == 9)
                    {
                        //DateTime firstDay = new DateTime(period.Year, period.Month, 1);
                        //DateTime lastDay = firstDay.AddMonths(1);
                        //mSelectAllErcotSourceAndSinkCRRCommand.Parameters["@StartDate"].Value = firstDay;
                        //mSelectAllErcotSourceAndSinkCRRCommand.Parameters["@EndDate"].Value = firstDay;
                        //mSelectAllErcotSourceAndSinkCRRCommand.CommandText = mSelectAllErcotSourceAndSinkCRRCommand.CommandText.Replace("@participant", account);
                        //reader = mSelectAllErcotSourceAndSinkCRRCommand.ExecuteReader();
                    ...
                    {
                        mSelectAllSPPSourceAndSinkCRRCommand.Parameters["@startdate"].Value = DateTime.Parse("6/1/" + CRRYear);
                        mSelectAllSPPSourceAndSinkCRRCommand.Parameters["@enddate"].Value = DateTime.Parse("5/31/" + (CRRYear + 1));
                        mSelectAllSPPSourceAndSinkCRRCommand.CommandText = mSelectAllSPPSourceAndSinkCRRCommand.CommandText.Replace("@participant", account);
                        reader = mSelectAllSPPSourceAndSinkCRRCommand.ExecuteReader();
                    }
                    else if (marketKey == 7)
                    {
                        mSelectAllCaisoSourceAndSinkCRRCommand.Parameters["@startdate"].Value = DateTime.Parse("1/1/" + CRRYear);
                        mSelectAllCaisoSourceAndSinkCRRCommand.Parameters["@enddate"].Value = DateTime.Parse("12/31/" + CRRYear);
                        mSelectAllCaisoSourceAndSinkCRRCommand.CommandText = mSelectAllCaisoSourceAndSinkCRRCommand.CommandText.Replace("@participant", account);
                        reader = mSelectAllCaisoSourceAndSinkCRRCommand.ExecuteReader();
                    }
                    while (reader.Read())
                    {
                        SourceSink sourcesink = new SourceSink();
        ...
        private MarketHours GetPeakOffPeakHours(DateTime period, int marketKey)
        {
            ...
            if (savedMarketHours == null || savedMarketHours.Year != CRRYear)
            {
                ...
                for (int i = 1; i <= 12; i++)
                {
                    DateTime startTime = marketKey == 9 || marketKey == 7 ? DateTime.Parse("1/1/" + CRRYear).AddHours(1).AddMonths(i - 1) : DateTime.Parse("6/1/" + CRRYear).AddHours(1).AddMonths(i - 1);
                    DateTime endTime = marketKey == 9 || marketKey == 7 ? DateTime.Parse("1/1/" + CRRYear).AddMonths(i) : DateTime.Parse("6/1/" + CRRYear).AddMonths(i);
        ...
        private Dictionary<int, Dictionary<int, Tuple<DailyValues, Dictionary<int, DailyValues>>>> GetFTRPjmData(int marketkey, long SourceId, long SinkId, DateTime startDate, DateTime endDate, string HedgeType, int CRRQuarterMonth, string auctionType)
        {
            ...
            if (HedgeType == "OBL")
            {
                if (CRRQuarterMonth != 0)
                {
                    ...
                    {
                        mSelectQuarterlyPriceCommand.CommandText = " select c.startdate ,  si.LMPOnPeak - b.LMPOnPeak , si.LMPOffPeak - b.LMPOffPeak  , c.PeakHrs , c.OffPeakHrs , c.PeriodKey  from pjm.CRRAuction a, pjm.CRRAuctionNodePrice b " +
                            " join period c on b.PeriodKey = c.PeriodKey join pjm.CRRAuctionNodePrice si on  b.CRRAuctionKey = si.CRRAuctionKey and b.PeriodKey = si.PeriodKey " +
                            " where  b.periodkey = (select periodkey from period where startdate = '" + CRRQuarterMonth.ToString() + "/1/" + year.ToString() + "' and periodtype = 'seasonal'  " +
                            " and marketkey = 1) and a.CRRAuctionKey = b.CRRAuctionKey and b.NodeKey  =  " + sourcenodekey.ToString() + " and  si.NodeKey = " + sinknodekey.ToString() + " and " +
                            " a.CRRAuctionKey = (select max(a.CRRAuctionkey) from pjm.CRRAuction a, pjm.CRRAuctionNodePrice b where  b.periodkey = " +
                            " (select periodkey from period where startdate =  '" + CRRQuarterMonth.ToString() + "/1/" + year.ToString() + "' and periodtype = 'seasonal' and marketkey = 1) and a.CRRAuctionKey = b.CRRAuctionKey) ";
                ...
                else if (auctionType == "Annual")
                {
                    ...
                    foreach (int year in yearList)
                    {
                       ...
                       " and startdate >= @startdate and enddate <= @enddate)  and a.PeriodKey = b.PeriodKey and a.CRRAuctionKey = b.CRRAuctionKey and a.PeriodKey = c.PeriodKey and a.CRRAuctionKey in(select MAX(CRRAuctionKey) from pjm.CRRAuction  where CRRAuctionStartDate = @startdate and CRRAuctionEndDate = @enddate  " +
            ...
            else
            {
                ...
                mSelectDailyValuesCommand.CommandText = "select p.marketdate, p1.AvgPeakCong - p.AvgPeakCong,p1.AvgOffpeakCong - p.AvgOffpeakCong,p1.Avg24Cong - p.Avg24Cong,p.PeakHours , "
                                                        ...
                                                        + "on p.MarketDate = p1.MarketDate and p.NodeKey = " + sourcenodekey + "    and p1.NodeKey = " + sinknodekey + " and p1.MarketDate >= '" + startDate + "'  and p1.MarketDate <= '" + endDate.AddMonths(1) + " ' and p.markettypecode = p1.markettypecode and p.markettypecode = 'rt' "
                                                        + "union "
        ...
        private Dictionary<int, Dictionary<int, Tuple<DailyValues, Dictionary<int, DailyValues>>>> GetFTRErcotData(int marketkey, long SourceId, long SinkId, DateTime startDate, DateTime endDate, string Hedgetype, int CRRQuarterMonth, string auctionType)
        {
                ...
                if (CRRQuarterMonth != 0)
                {
                    ...
                    foreach (int year in yearList)
                    {
                        mSelectQuarterlyPriceCommandErcot.CommandText = " select c.startdate ,  si.LMPOnPeak - b.LMPOnPeak , si.LMPOffPeak - b.LMPOffPeak  , c.PeakHrs , c.OffPeakHrs , c.PeriodKey, si.PeakWE - b.PeakWE, c.PEAKWE from crrauction a,CRRAuctionNodePrice b " +
                            ...
                            " (select periodkey from period where startdate =  '" + CRRQuarterMonth.ToString() + "/1/" + year.ToString() + "' and periodtype = 'seasonal' and marketkey = 9) and a.CRRAuctionKey = b.CRRAuctionKey) ";
            ...
            else
            {
                ...
                mSelectDailyValuesCommand.CommandText = "select p.marketdate, p1.AvgPeakLMP - p.AvgPeakLMP,p1.AvgOffpeakLMP - p.AvgOffpeakLMP,p1.Avg24LMP - p.Avg24LMP,p.PeakHours , "
                                ...
                                + " from NodeLMPDailysOPT (nolock) where SourceNodeKey = " + sourcenodekey + " and SinkNodeKey = " + sinknodekey + " and MarketDate>= '" + startDate + "' and MarketDate<= '" + endDate.AddMonths(1) + "'  "
                                + " union "
                                + " select MarketDate, AvgPeakLMP, AvgOffpeakLMP, Avg24LMP, PeakHours, OffpeakHours, MarketTypeCode, AvgPeakWELMP,PeakWEHours "
                                + " from NodeLMPDailysOption (nolock) where SourceNodeKey = " + sourcenodekey + " and SinkNodeKey = " + sinknodekey + " and MarketDate>= '" + startDate + "' and MarketDate<= '" + endDate.AddMonths(1) + "'   order by p.markettypecode desc, "
                                + " p.MarketDate desc ";
                mSelectDailyValuesCommand.CommandTimeout = 300000;
                reader = mSelectDailyValuesCommand.ExecuteReader();
                while (reader.Read())
        ...
        public Dictionary<int, Dictionary<int, Tuple<DailyValues, Dictionary<int, DailyValues>>>> FillFTRPathData(int Marketkey, long SourceId, long SinkId, DateTime startDate, DateTime endDate, string periodType, string Hedgetype, int CRRQuarterMonth = 0, string auctionType = "Monthly")
        {
            if (Marketkey == 1)
            {
                yearHash = GetFTRPjmData(Marketkey, SourceId, SinkId, startDate, endDate, Hedgetype, CRRQuarterMonth = 0, auctionType = "Monthly");
            }
            else if (Marketkey == 9)
            {
                yearHash = GetFTRErcotData(Marketkey, SourceId, SinkId, startDate, endDate, Hedgetype, CRRQuarterMonth = 0, auctionType = "Monthly");
            }

            return yearHash;
        }

        //public Dictionary<int, Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>> FillFTRPathData1(int Marketkey, long SourceId, long SinkId, DateTime startDate, DateTime endDate, List<int> selectedMonths, string Hedgetype, string classType)
        //{
        //    Dictionary<int, Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>> yearHash1 = null;

        //    if (Marketkey == 9)
        //    {
        //        yearHash1 = GetFTRErcotData1(Marketkey, SourceId, SinkId, selectedMonths, Hedgetype, classType);
        //        PopulateFlattenedMonthlyValues(yearHash1, selectedMonths);
        //    }
        ...
        public List<MonthlyValues> FillFTRPathData1(int Marketkey, long SourceId, long SinkId, DateTime startDate, DateTime endDate, List<int> selectedMonths, string Hedgetype, string classType)
        {
            Dictionary<int, Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>> yearHash1 = null;
            ...
            {
                yearHash1 = GetFTRErcotData1(Marketkey, SourceId, SinkId, selectedMonths, Hedgetype, classType);
                return PopulateFlattenedMonthlyValues(yearHash1, selectedMonths);
            }

            return new List<MonthlyValues>(); // Return an empty list if Marketkey is not 9
        }



        private Dictionary<int, Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>> GetFTRErcotData1(
     int marketkey,
     long SourceId,
     long SinkId,
     List<int> selectedMonths,
     string Hedgetype, string classType)
        {
            String culture = "lt-LT";
            Thread.CurrentThread.CurrentCulture = CultureInfo.CreateSpecificCulture(culture);
            InitDB();
            ...
            Dictionary<int, Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>> yearHash1 =
                new Dictionary<int, Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>>();
            ...
            for (int year = DateTime.Now.Year; year > DateTime.Now.Year - 3; year--)
            {
                foreach (int month in selectedMonths)
                {
                    ...
                    {
                        command.Connection = VayuDBConnection;

                        if (Hedgetype == "OBL")
                        {
                            command.CommandText = @"select c.StartDate, a.LMPOnPeak - b.LMPOnPeak, a.LMPOffPeak - b.LMPOffPeak , c.PeakHrs, c.OffPeakHrs, c.periodkey, a.PeakWE - b.PeakWE, c.PEAKWE 
                                        from CRRAuctionNodePrice a, CRRAuctionNodePrice b, period c 
                                        where a.nodekey = @source 
                                        and a.periodkey in (select PeriodKey from period where marketkey = 9 and periodtype = 'Monthly' and startdate >= @startdate and enddate <= @enddate)  
                                        and b.nodekey = @sink 
                                        and b.periodkey in (select PeriodKey from period where marketkey = 9 and periodtype = 'Monthly' and startdate >= @startdate and enddate <= @enddate)  
                                        and a.PeriodKey = b.PeriodKey 
                                        and a.CRRAuctionkey = b.CRRAuctionkey 
                                        and a.PeriodKey = c.PeriodKey 
                                        and a.CRRAuctionkey in(select MAX(CRRAuctionKey) from crrauction where CRRAuctionStartDate = @startdate and CRRAuctionEndDate = @enddate) 
                                        order by a.periodkey, a.CRRAuctionkey";
                        }
                        else if (Hedgetype == "OPT")
                        {
                            command.CommandText = "select c.StartDate,  a.PeakWD, a.OffPeak , c.PeakHrs, c.OffPeakHrs, c.periodkey, a.PeakWE, C.PeakWE from CRRAuctionOptionPrice a join period c "
                                                       + "on a.StartDate = c.StartDate and a.EndDate = c.EndDate "
                                                       + "where a.Sourcekey = @source and a.Sinkkey = @sink  and c.StartDate >= @startdate "
                        ...
                        command.Parameters.AddWithValue("@source", sourcenodekey);
                        command.Parameters.AddWithValue("@sink", sinknodekey);
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                {
                                    StartDate = reader.IsDBNull(0) ? DateTime.MinValue : reader.GetDateTime(0),
                                    PricePeak = reader.IsDBNull(1) ? (double?)null : (double?)reader.GetDecimal(1),
                                    PriceOffPeak = reader.IsDBNull(2) ? (double?)null : (double?)reader.GetDecimal(2),
                                    PeakHours = reader.IsDBNull(3) ? 0 : reader.GetInt32(3),
                                    OffPeakHours = reader.IsDBNull(4) ? 0 : reader.GetInt32(4),
                                    PeriodKey = reader.IsDBNull(5) ? 0 : reader.GetInt32(5),
                                    PriceWE = reader.IsDBNull(6) ? (double?)null : (double?)reader.GetDecimal(6),
                                    PeakWE = reader.IsDBNull(7) ? 0 : reader.GetInt32(7)
                                };

                                if (!yearHash1.ContainsKey(year))
                                {
                                    yearHash1[year] = new Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>();
                                }

                                yearHash1[year][month] = Tuple.Create(monthlyValues, new Dictionary<int, MonthlyValues>());
                            }
                        }
                    }
                }
            ...
            //using (SqlConnection cmd = new VayuDBConnection().GetInstance().GetSqlConnection())
            //{
            //    if (cmd.State != System.Data.ConnectionState.Open)
            //    {
            //        cmd.Open();
            ...
            //    foreach (var yearEntry in yearHash1)
            //    {
            //        int year = yearEntry.Key;
            //        foreach (int month in selectedMonths)
            //        {
            //            if (!yearHash1[year].ContainsKey(month))
            //            {
            //                continue; 
            //            }
            ...
            //            using (SqlCommand mSelectDailyValuesCommand = cmd.CreateCommand())
            //            {
            //                mSelectDailyValuesCommand.CommandText = "select p.marketdate, p1.AvgPeakLMP - p.AvgPeakLMP,p1.AvgOffpeakLMP - p.AvgOffpeakLMP,p1.Avg24LMP - p.Avg24LMP,p.PeakHours , "
            //                    + " p.OffpeakHours, p.markettypecode, p1.AvgPeakWELMP - p.AvgPeakWELMP ,p.PeakWEHours from NodeLMPDailys p join NodeLMPDailys p1 "
            //                    + " on p.MarketDate = p1.MarketDate and p.NodeKey = @source and p1.NodeKey = @sink and p.MarketDate >= @startdate and p.MarketDate <= @enddate "
            //                    + " and p1.MarketDate >= @startdate and p1.MarketDate <= @enddate and p.markettypecode = p1.markettypecode and p.markettypecode = 'rt' "
            //                    + " union "
            //                    + " select MarketDate, AvgPeakLMP, AvgOffpeakLMP, Avg24LMP, PeakHours, OffpeakHours, MarketTypeCode, AvgPeakWELMP,PeakWEHours "
            //                    + " from NodeLMPDailysOPT (nolock) where SourceNodeKey = @source and SinkNodeKey = @sink and MarketDate>= @startdate and MarketDate<= @enddate "
            //                    + " union "
            //                    + " select MarketDate, AvgPeakLMP, AvgOffpeakLMP, Avg24LMP, PeakHours, OffpeakHours, MarketTypeCode, AvgPeakWELMP,PeakWEHours "
            //                    + " from NodeLMPDailysOption (nolock) where SourceNodeKey = @source and SinkNodeKey = @sink and MarketDate>= @startdate and MarketDate<= @enddate order by p.markettypecode desc, "
            //                    + " p.MarketDate desc ";

            //                mSelectDailyValuesCommand.Parameters.AddWithValue("@startdate", startDate);
            //                mSelectDailyValuesCommand.Parameters.AddWithValue("@enddate", endDate.AddMonths(1));
            //                mSelectDailyValuesCommand.Parameters.AddWithValue("@source", sourcenodekey);
            //                mSelectDailyValuesCommand.Parameters.AddWithValue("@sink", sinknodekey);

            //                using (SqlDataReader reader = mSelectDailyValuesCommand.ExecuteReader())
            //                {
            //                    double totalDAPeakCong = 0;
            //                    double totalDAOffPeakCong = 0;
            //                    double totalDA24Cong = 0;
            //                    double totalDAPeakWECong = 0;
            //                    int daysWithData = 0;
            ...
            //                            if (yearHash1.ContainsKey(marketdate.Year) && yearHash1[marketdate.Year].ContainsKey(marketdate.Month))
            //                            {
            //                                var monthData = yearHash1[marketdate.Year][marketdate.Month];
            //                                var daysHash = monthData.Item2;

            //                                if (!daysHash.ContainsKey(marketdate.Day))
            //                                {
            //                                    daysHash[marketdate.Day] = new MonthlyValues();
            //                                }

            //                                MonthlyValues congValues = daysHash[marketdate.Day];
            ...
            //                                    totalDAPeakCong += congValues.DAPeakCong ?? 0;
            //                                    totalDAOffPeakCong += congValues.DAOffPeakCong ?? 0;
            //                                    totalDA24Cong += congValues.DA24Cong ?? 0;
            //                                    totalDAPeakWECong += congValues.DAPeakWECong ?? 0;
            ...
            //                    if (daysWithData > 0)
            //                    {
            //                        var monthData = yearHash1[year][month];
            //                        var monthlyValues = monthData.Item1;

            //                        monthlyValues.DAOffPeakCong = totalDAOffPeakCong / daysWithData;
            //                        monthlyValues.DA24Cong = totalDA24Cong / daysWithData;

            //                        if (totalWeekdays > 0)
            //                        {
            //                            monthlyValues.DAPeakCong = totalDAPeakCong / totalWeekdays;
            //                        }
            //                        else
            //                        {
            //                            monthlyValues.DAPeakCong = null;
            //                        }

            //                        if (totalWeekendDays > 0)
            //                        {
            //                            monthlyValues.DAPeakWECong = totalDAPeakWECong / totalWeekendDays;
            //                        }
            //                        else
            //                        {
            //                            monthlyValues.DAPeakWECong = null;
            //                        }
            //                    }
            //                }
            //            }
            ...
            using (SqlConnection cmd = new VayuDBConnection().GetInstance().GetSqlConnection())
            {
                ...
                // Create a comma-separated string of selected months
                string selectedMonthsString = string.Join(",", selectedMonths);

                // Loop through the last 3 years
                for (int year = DateTime.Now.Year; year > DateTime.Now.Year - 3; year--)
                {
                    foreach (int month in selectedMonths) // Loop through selected months
                    {
                        DateTime startDate = new DateTime(year, month, 1); // Start of the month
                        DateTime endDate = startDate.AddMonths(1).AddDays(-1); // End of the month

                        using (SqlCommand mSelectDailyValuesCommand = cmd.CreateCommand())
                        {
                            mSelectDailyValuesCommand.CommandText = $@"
                    select p.marketdate, p1.AvgPeakLMP - p.AvgPeakLMP, p1.AvgOffpeakLMP - p.AvgOffpeakLMP, p1.Avg24LMP - p.Avg24LMP, p.PeakHours, 
                    p.OffpeakHours, p.markettypecode, p1.AvgPeakWELMP - p.AvgPeakWELMP, p.PeakWEHours 
                    from NodeLMPDailys p 
                    join NodeLMPDailys p1 on p.MarketDate = p1.MarketDate 
                    where p.NodeKey = @source 
                    and p1.NodeKey = @sink 
                    and p.MarketDate >= @startdate 
                    and p.MarketDate <= @enddate 
                    and MONTH(p.MarketDate) IN ({selectedMonthsString}) 
                    and p.markettypecode = 'rt' 
                    union 
                    select MarketDate, AvgPeakLMP, AvgOffpeakLMP, Avg24LMP, PeakHours, OffpeakHours, MarketTypeCode, AvgPeakWELMP, PeakWEHours 
                    from NodeLMPDailysOPT (nolock) 
                    where SourceNodeKey = @source 
                    and SinkNodeKey = @sink 
                    and MarketDate >= @startdate 
                    and MarketDate <= @enddate 
                    and MONTH(MarketDate) IN ({selectedMonthsString}) 
                    union 
                    select MarketDate, AvgPeakLMP, AvgOffpeakLMP, Avg24LMP, PeakHours, OffpeakHours, MarketTypeCode, AvgPeakWELMP, PeakWEHours 
                    from NodeLMPDailysOption (nolock) 
                    where SourceNodeKey = @source 
                    and SinkNodeKey = @sink 
                    and MarketDate >= @startdate 
                    and MarketDate <= @enddate 
                    and MONTH(MarketDate) IN ({selectedMonthsString}) 
                    order by p.markettypecode desc, p.MarketDate desc";

                            mSelectDailyValuesCommand.Parameters.AddWithValue("@startdate", startDate);
                            mSelectDailyValuesCommand.Parameters.AddWithValue("@enddate", endDate);
                            mSelectDailyValuesCommand.Parameters.AddWithValue("@source", sourcenodekey);
                            mSelectDailyValuesCommand.Parameters.AddWithValue("@sink", sinknodekey);
                            ...
                            using (SqlDataReader reader = mSelectDailyValuesCommand.ExecuteReader())
                            {
                                ...
                                // Calculate total weekdays and weekend days for the current month
                                for (int day = 1; day <= DateTime.DaysInMonth(year, month); day++)
                                {
                                    DateTime date = new DateTime(year, month, day);
                                    if (date.DayOfWeek == DayOfWeek.Saturday || date.DayOfWeek == DayOfWeek.Sunday)
        ...
        private List<MonthlyValues> PopulateFlattenedMonthlyValues(
    Dictionary<int, Dictionary<int, Tuple<MonthlyValues, Dictionary<int, MonthlyValues>>>> yearHash1,
    List<int> selectedMonths)
        {
            List<MonthlyValues> flattenedMonthlyValuesList = new List<MonthlyValues>();
            int currentYear = DateTime.Now.Year;

            MonthlyValues yearlyMonthlyValues = new MonthlyValues();
            ...
            foreach (var yearEntry in yearHash1)
            {
                ...
                foreach (var monthEntry in yearEntry.Value)
                {
                    ...
                    var monthlyValues = monthEntry.Value.Item1;

                    if (year == currentYear)
                    {
                        if (month == selectedMonths[0])
                        {
                            yearlyMonthlyValues.PricePeakYear1Month1 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear1Month1 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear1Month1 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear1Month1 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear1Month1 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear1Month1 = monthlyValues.DAPeakWECong;
                        }
                        else if (month == selectedMonths[1])
                        {
                            yearlyMonthlyValues.PricePeakYear1Month2 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear1Month2 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear1Month2 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear1Month2 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear1Month2 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear1Month2 = monthlyValues.DAPeakWECong;
                        }
                        else if (month == selectedMonths[2])
                        {
                            yearlyMonthlyValues.PricePeakYear1Month3 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear1Month3 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear1Month3 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear1Month3 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear1Month3 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear1Month3 = monthlyValues.DAPeakWECong;
                        }
                    }
                    else if (year == currentYear - 1)
                    {
                        if (month == selectedMonths[0])
                        {
                            yearlyMonthlyValues.PricePeakYear2Month1 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear2Month1 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear2Month1 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear2Month1 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear2Month1 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear2Month1 = monthlyValues.DAPeakWECong;
                        }
                        else if (month == selectedMonths[1])
                        {
                            yearlyMonthlyValues.PricePeakYear2Month2 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear2Month2 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear2Month2 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear2Month2 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear2Month2 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear2Month2 = monthlyValues.DAPeakWECong;
                        }
                        else if (month == selectedMonths[2])
                        {
                            yearlyMonthlyValues.PricePeakYear2Month3 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear2Month3 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear2Month3 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear2Month3 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear2Month3 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear2Month3 = monthlyValues.DAPeakWECong;
                        }
                    }
                    else if (year == currentYear - 2)
                    {
                        if (month == selectedMonths[0])
                        {
                            yearlyMonthlyValues.PricePeakYear3Month1 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear3Month1 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear3Month1 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear3Month1 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear3Month1 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear3Month1 = monthlyValues.DAPeakWECong;
                        }
                        else if (month == selectedMonths[1])
                        {
                            yearlyMonthlyValues.PricePeakYear3Month2 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear3Month2 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear3Month2 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear3Month2 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear3Month2 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear3Month2 = monthlyValues.DAPeakWECong;
                        }
                        else if (month == selectedMonths[2])
                        {
                            yearlyMonthlyValues.PricePeakYear3Month3 = monthlyValues.PricePeak;
                            yearlyMonthlyValues.PriceOffPeakYear3Month3 = monthlyValues.PriceOffPeak;
                            yearlyMonthlyValues.PriceWEYear3Month3 = monthlyValues.PriceWE;

                            yearlyMonthlyValues.DAPeakCongYear3Month3 = monthlyValues.DAPeakCong;
                            yearlyMonthlyValues.DAOffPeakCongYear3Month3 = monthlyValues.DAOffPeakCong;
                            yearlyMonthlyValues.DAPeakWECongYear3Month3 = monthlyValues.DAPeakWECong;
                        }
                    }
                }
            }

            flattenedMonthlyValuesList.Add(yearlyMonthlyValues);

            return flattenedMonthlyValuesList;
        }
