...
namespace Vayu.CRRCreditCalculationService
{
    class CRRCreditCalculationServer : ICRRCredit
    {
        ...
        public void Connect()
        {
            using (ServiceHost host = new ServiceHost(typeof(CRRCreditCalculationServer), new Uri(CommonAccessLibrary.ServiceConnections.GetCreditCRRService())))
            {
        ...
        private void InitDB()
        {
            mConnection90 = new SqlConnection(Vayu.CommonAccessLibrary.DBConnectionCredentials.GetERCOTDBConnection());
        }
