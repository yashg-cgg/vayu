...
namespace Vayu.NodePriceService
{
    ...
    class LMPServer : ILMP
    {
        ...
        public Node[] GetAllUptoPriceDateTime(int market, bool isDA, DateTime startDateTime, DateTime endDateTime, bool isFiveMin, bool onlyPrice)
        {
            ...
            if (market == 1)
            {
                sourceSinkHash = dictPJMSourceSinkHash;
            }
            else if (market == 9)
            {
        ...
        public void Connect()
        {
            using (ServiceHost host = new ServiceHost(typeof(LMPServer), new Uri("net.tcp://localhost:7000")))
            {
                ...
                {
                    host.Open();
                    Console.WriteLine("LMP server successfully opened port 7000.");
                    Console.ReadLine();
                    host.Close();
                }
                catch (Exception e)
        ...
        private void LoadDB()
        {
            ...
            //
            cmdSelectMappingCommand = new SqlCommand();
            cmdSelectMappingCommand.CommandText = "select AggregatePNodeID, BusPNodeID, Factor from dbo.PJMAggregateFactors where Date <= @Date order by date desc";
            cmdSelectMappingCommand.Parameters.Add("@Date", SqlDbType.DateTime);
            cmdSelectMappingCommand.Connection = vayuDBConnection;
            //
            ...
            cmdSelectErcotSourceSinkNodeCommand.CommandText = "select src.SourceNodeKey, src.SourceName, n.ExternalNodeId as SourceExternalNodeID, n.NodeTypeKey as SourceNodeTypeKey, "
                                            + "n.Zone as SourceNodeZone, sink.SinkNodeKey, sink.SinkName, n2.ExternalNodeID as SinkExternalNodeID, "
                                            + "n2.NodeTypeKey as SinkNodeTypeKey, n2.Zone as SinkNodeZone "
                                            + "from Vayu_ercot..EESPathList src (NOLOCK) "
                                            + "inner join Vayu_ercot..Node n (NOLOCK) on n.NodeKey = src.SourceNodeKey "
                                            + "inner join Vayu_ercot..EESPathList sink (NOLOCK) on sink.SinkNodeKey = src.SinkNodeKey and sink.EESPathListKey = src.EESPathListKey "
                                            + "inner join Vayu_ercot..Node n2 (NOLOCK) on n2.NodeKey = sink.SinkNodeKey where n.MarketKey = @market and n2.MarketKey = @market and src.MarketKey = @market "
                                            + "and sink.MarketKey = @market order by n.NodeName ";
            cmdSelectErcotSourceSinkNodeCommand.Parameters.AddWithValue("@market", "market");
            //cmdSelectErcotSourceSinkNodeCommand.Connection = SigmaDBConnection;
            ...
            cmdSelectErcotPriceRTCommand.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED Select Node.NodeKey, node.ExternalNodeID, LMP, MarketDateTime, " +
                                            "node.nodetypekey, node.Nodename, congestion, loss From Vayu_ercot..NodeLmph (NOLOCK) Join Vayu_ercot..Node on Node.NodeKey = Vayu_ercot..NodeLMPH.NodeKey Where " +
            ...
            cmdSelectPriceERCOTFiveMinRTCommand.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED Select Node.NodeKey, node.ExternalNodeID, LMP, MarketDateTime, " +
                                            "node.nodetypekey, node.Nodename, congestion, loss From Vayu_ercot..NodeLmp (NOLOCK) Join Vayu_ercot..Node on Node.NodeKey = NodeLMP.NodeKey Where " +
                                              "MarketDateTime >= @Start And MarketDateTime < @End And Node.NodeKey = (select min(NodeKey) from Vayu_ercot..Node where " +
                                              "externalnodeid = @externalnodeid) SET TRANSACTION ISOLATION LEVEL READ COMMITTED";
            ...
            //cmdSelectPriceERCOTFiveMinRTCommand.Connection = SigmaDBConnection;
            ...
            cmdSelectAllErcotPriceRTCommand.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED Select Node.NodeKey, node.ExternalNodeID, LMP, MarketDateTime, " +
                                                    "node.nodetypekey, Node.NodeName, congestion, loss From Vayu_ercot..NodeLmph (NOLOCK) Join Vayu_ercot..Node on Node.NodeKey = NodeLMPH.NodeKey Where " +
                                                    "MarketDateTime >= @Start And MarketDateTime < @End And MarketKey = @MarketKey SET TRANSACTION ISOLATION LEVEL " +
                                                    "READ COMMITTED";
            ...
            cmdSelectERCOTAllNodesFiveMinRTCommand.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED Select Node.NodeKey, node.ExternalNodeID, LMP, MarketDateTime, " +
                                                             "node.nodetypekey, Node.NodeName, congestion, loss From Vayu_ercot..NodeLmp (NOLOCK) Join Vayu_ercot..Node on Node.NodeKey = Vayu_ercot..NodeLMP.NodeKey  ";
            ...
            cmdSelectErcotAllNodesDACommand.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED Select Node.NodeKey, node.ExternalNodeID, LMP, MarketDateTime, " +
                                                    "node.nodetypekey, Node.NodeName, congestion, loss From Vayu_ercot..NodeDALmph (NOLOCK) Join Vayu_ercot..Node on Node.NodeKey = Vayu_ercot..NodeDALMPH.NodeKey Where " +
                                                    "MarketDateTime >= @Start And MarketDateTime < @End SET TRANSACTION ISOLATION LEVEL READ COMMITTED";
                    ...
                    else
                    {
                        if (isFiveMin)
                        {
                            ...
                            cmdSelectERCOTAllNodesFiveMinRTCommand.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED Select Node.NodeKey, node.ExternalNodeID, LMP,  DATEADD(HOUR , MarketHour , DATEADD(MINUTE , MarketMin , MarketDate)) as marketdatetime , " +
                                         "node.nodetypekey, node.Nodename, 0, 0 From Vayu_ercot..NodeLmpMin (NOLOCK) Join Vayu_ercot..Node on Node.NodeKey = NodeLMPMin.NodeKey Where " +
        ...
        public Double GetEnergyPrice(DateTime timeValue)
        {
            ...
            cmd.CommandText = "select SystemLambda from  Vayu_ercot..RTSystemLambda where DeliveryDate between '"+dtval1+"' and '"+dtval2+"';";
            cmd.Connection = vayuDBConnection;
