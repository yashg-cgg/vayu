...
{
    public class CalculateMinMaxRTFall
    {
        ...
        private DataTable mDtCalculateMinMaxRTFall = new DataTable();
        private DataRow dr = null;
        public CalculateMinMaxRTFall()
        {
            InitDB();
            ...
            GetEESPaths();
            mDtCalculateMinMaxRTFall.Clear();
        ...
        private void SaveInDB(DataTable tempDt)
        {
            ...
            using (SqlBulkCopy bkLmpH = new SqlBulkCopy(mConnection, SqlBulkCopyOptions.TableLock, transaction))
            {
                try
                {
                    bkLmpH.DestinationTableName = "[dbo].[BlockAlgoMinMaxFallTest]";
                    bkLmpH.ColumnMappings.Add("SourceName", "SourceName");
                    bkLmpH.ColumnMappings.Add("SinkName", "SinkName");
                    bkLmpH.ColumnMappings.Add("RTMinFall", "RTMinFall");
                    bkLmpH.ColumnMappings.Add("RTMaxFall", "RTMaxFall");
                    bkLmpH.WriteToServer(tempDt);
                    SqlCommand cmdUpdatenodelmph = new SqlCommand("[dbo].[PJMDeleteDupsBlockAlgofall]", mConnection, transaction);
            ...
            try
            {
                ...
                using (SqlBulkCopy bkLmpH = new SqlBulkCopy(mConnection, SqlBulkCopyOptions.TableLock, transaction2))
                {
                    try
                    {
                        ...
                        {
                            SqlCommand cmdUpdateNodeLmpMin = new SqlCommand("[PJM].[UpMergeBlockAlgofall]", mConnection, transaction2);
        ...
        private void SaveInProd()
        {
            ...
            mConnection.Open();
            SqlDataAdapter da = new SqlDataAdapter("select a.SourceName,a.SinkName,MinValue as RTMinFall,MaxValue as RTMaxFall  from BlockAlgoMinMaxFalltemp a inner join BlockAlgoMinMaxFall b on a.SourceName=b.SourceName and a.SinkName=b.SinkName   inner join BlockAlgoMinMaxSummer c on c.SourceName=a.SourceName and c.SinkName=a.SinkName   CROSS APPLY (SELECT MIN(d) MinValue FROM (VALUES (a.RTMinFall), (a.RTMaxFall), (b.RTMinFall),(b.RTMaxFall)) AS MI(d)) MI   CROSS APPLY (SELECT MAX(e) MaxValue FROM (VALUES (a.RTMinFall), (a.RTMaxFall), (b.RTMinFall),(b.RTMaxFall)) AS MI(e)) MX", mConnection);
            da.Fill(mDtCalculateMinMaxRTFall);
            ...
            using (SqlBulkCopy bkLmpH = new SqlBulkCopy(mConnection, SqlBulkCopyOptions.TableLock, transaction))
            {
                try
                {
                    //bkLmpH.DestinationTableName = "BlockAlgoMinMaxFalltemp";
                    ...
                    //bkLmpH.ColumnMappings.Add("RTMinFall", "RTMinFall");
                    //bkLmpH.ColumnMappings.Add("RTMaxFall", "RTMaxFall");
                    //bkLmpH.WriteToServer(mDtCalculateMinMaxRTFall);
                    SqlCommand cmdUpdatenodelmph = new SqlCommand("[PJM].[UpMergeFinalBlockAlgoFall]", mConnection, transaction);
                    cmdUpdatenodelmph.CommandType = CommandType.StoredProcedure;
                    cmdUpdatenodelmph.CommandTimeout = 30000;
                    cmdUpdatenodelmph.ExecuteNonQuery();
                    transaction.Commit();
            ...
            try
            {
                ...
                if (hourlyPivotData.Where(a => a.RowType == null).Count() <= 0)
                {
                    ...
                    if (sortTempHourlyPivotList.Count > 0)
                    {
                        ...
                        dr = mDtCalculateMinMaxRTFall.NewRow();
                        dr["SourceName"] = source;
                        dr["SinkName"] = sink;
                        dr["RTMinFall"] = minHourlySummaryPivot.HourDataList.Min(x => x.Value);
                        dr["RTMaxFall"] = maxHourlySummaryPivot.HourDataList.Max(x => x.Value);
                        mDtCalculateMinMaxRTFall.Rows.Add(dr);
                        if (mDtCalculateMinMaxRTFall.Rows.Count > 0)
                            SaveInDB(mDtCalculateMinMaxRTFall);
                    }
        ...
        private void InitDB()
        {
           ...
           // mSelectPathsCommand.CommandText = "select distinct sourcenodekey, sinknodekey from eespathlist where SourceName='AMOS AM1' and sinkname='BLUE RIDGE'";
            mSelectPathsCommand.Connection = mConnection;
            mDtCalculateMinMaxRTFall = new DataTable();
            mDtCalculateMinMaxRTFall.Columns.Add("SourceName", typeof(string));
            mDtCalculateMinMaxRTFall.Columns.Add("SinkName", typeof(string));
            mDtCalculateMinMaxRTFall.Columns.Add("RTMinFall", typeof(double));
            mDtCalculateMinMaxRTFall.Columns.Add("RTMaxFall", typeof(double));

            cmdDeleteCommand = new SqlCommand();
            cmdDeleteCommand.CommandText = "truncate table BlockAlgoMinMaxFallTest";
        ...
        private List<Node> GetDarts(SourceSinkData sourceSinkData)
        {
            ...
            try
            {
                ...
                mDates2013.sDateTime = new DateTime(2013, 10, 01);
                mDates2013.eDateTime = new DateTime(2013, 11, 30);
                mDateTimeList.Add(mDates2013);

                Dates mDates2014 = new Dates();
                mDates2014.sDateTime = new DateTime(2014, 10, 01);
                mDates2014.eDateTime = new DateTime(2014, 11, 30);
                mDateTimeList.Add(mDates2014);

                Dates mDates2015 = new Dates();
                mDates2015.sDateTime = new DateTime(2015, 10, 01);
                mDates2015.eDateTime = new DateTime(2015, 11, 30);
                mDateTimeList.Add(mDates2015);

                Dates mDates2016 = new Dates();
                mDates2016.sDateTime = new DateTime(2016, 10, 01);
                mDates2016.eDateTime = new DateTime(2016, 11, 30);
                mDateTimeList.Add(mDates2016);

                Dates mDates2017 = new Dates();
                mDates2017.sDateTime = new DateTime(2017, 10, 01);
                mDates2017.eDateTime = new DateTime(2017, 11, 30);
                ...
                foreach (Dates itemdates in mDateTimeList)
                {
                    ...
                    lmpList = GetChartData(itemdates.sDateTime, itemdates.eDateTime, sourceSinkData.Source, sourceSinkData.Sink);
