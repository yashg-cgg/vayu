...
{
    public class CalculateMinMaxRTSpring
    {
        ...
        private DataTable mDtCalculateMinMaxRTSpring = new DataTable();
        private DataRow dr = null;
        public CalculateMinMaxRTSpring(DateTime mDates)
        {
            InitDB();
            ...
            GetEESPaths();
            mDtCalculateMinMaxRTSpring.Clear();
        ...
        private void SaveInProd()
        {
            ...
            mConnection.Open();
            SqlDataAdapter da = new SqlDataAdapter("select a.SourceName,a.SinkName,MinValue as RTMinSpring,MaxValue as RTMaxSpring  from BlockAlgoMinMaxSpringtemp a inner join BlockAlgoMinMaxSpring b on a.SourceName=b.SourceName and a.SinkName=b.SinkName   inner join BlockAlgoMinMaxSummer c on c.SourceName=a.SourceName and c.SinkName=a.SinkName   CROSS APPLY (SELECT MIN(d) MinValue FROM (VALUES (a.RTMinSpring), (a.RTMaxSpring), (b.RTMinSpring),(b.RTMaxSpring)) AS MI(d)) MI   CROSS APPLY (SELECT MAX(e) MaxValue FROM (VALUES (a.RTMinSpring), (a.RTMaxSpring), (b.RTMinSpring),(b.RTMaxSpring)) AS MI(e)) MX", mConnection);
            da.Fill(mDtCalculateMinMaxRTSpring);
            ...
            using (SqlBulkCopy bkLmpH = new SqlBulkCopy(mConnection, SqlBulkCopyOptions.TableLock, transaction))
            {
                try
                {
                    //bkLmpH.DestinationTableName = "BlockAlgoMinMaxSpringtemp";
                    //bkLmpH.BulkCopyTimeout = 30000;
                    //bkLmpH.BatchSize = 1500;
                    //bkLmpH.ColumnMappings.Add("SourceName", "SourceName");
                    //bkLmpH.ColumnMappings.Add("SinkName", "SinkName");create
                    //bkLmpH.ColumnMappings.Add("RTMinSpring", "RTMinSpring");
                    //bkLmpH.ColumnMappings.Add("RTMaxSpring", "RTMaxSpring");
                    //bkLmpH.WriteToServer(mDtCalculateMinMaxRTSpring);
                    SqlCommand cmdUpdatenodelmph = new SqlCommand("[PJM].[UpMergeFinalBlockAlgoSpring]", mConnection, transaction);
        ...
        private void SaveInDB(DataTable tempDt)
        {
            ...
            using (SqlBulkCopy bkLmpH = new SqlBulkCopy(mConnection, SqlBulkCopyOptions.TableLock, transaction))
            {
                try
                {
                    bkLmpH.DestinationTableName = "[dbo].[BlockAlgoMinMaxSpringTest]";
                    bkLmpH.ColumnMappings.Add("SourceName", "SourceName");
                    bkLmpH.ColumnMappings.Add("SinkName", "SinkName");
                    bkLmpH.ColumnMappings.Add("RTMinSpring", "RTMinSpring");
                    bkLmpH.ColumnMappings.Add("RTMaxSpring", "RTMaxSpring");
                    bkLmpH.WriteToServer(tempDt);
                    SqlCommand cmdUpdatenodelmph = new SqlCommand("[dbo].[PJMDeleteDupsBlockAlgoSpring]", mConnection, transaction);
                    cmdUpdatenodelmph.CommandType = CommandType.StoredProcedure;
                    cmdUpdatenodelmph.CommandTimeout = 30000;
                    cmdUpdatenodelmph.ExecuteNonQuery();
                    transaction.Commit();
            ...
            try
            {
                ...
                using (SqlBulkCopy bkLmpH = new SqlBulkCopy(mConnection, SqlBulkCopyOptions.TableLock, transaction2))
                {
                    try
                    {
                        ...
                        {
                            SqlCommand cmdUpdateNodeLmpMin = new SqlCommand("[PJM].[UpMergeBlockAlgoSpring]", mConnection, transaction2);
            ...
            try
            {
                ...
                if (hourlyPivotData.Where(a => a.RowType == null).Count() <= 0)
                {
                    ...
                    if (sortTempHourlyPivotList.Count > 0)
                    {
                        ...
                        dr = mDtCalculateMinMaxRTSpring.NewRow();
                        dr["SourceName"] = source;
                        dr["SinkName"] = sink;
                        dr["RTMinSpring"] = minHourlySummaryPivot.HourDataList.Min(x => x.Value);
                        dr["RTMaxSpring"] = maxHourlySummaryPivot.HourDataList.Max(x => x.Value);
                        mDtCalculateMinMaxRTSpring.Rows.Add(dr);
                        if (mDtCalculateMinMaxRTSpring.Rows.Count > 0)
                            SaveInDB(mDtCalculateMinMaxRTSpring);
                    }
        ...
        private void InitDB()
        {
            ...
            mDtCalculateMinMaxRTSpring = new DataTable();
            mDtCalculateMinMaxRTSpring.Columns.Add("SourceName", typeof(string));
            mDtCalculateMinMaxRTSpring.Columns.Add("SinkName", typeof(string));
            mDtCalculateMinMaxRTSpring.Columns.Add("RTMinSpring", typeof(double));
            mDtCalculateMinMaxRTSpring.Columns.Add("RTMaxSpring", typeof(double));

            cmdDeleteCommand = new SqlCommand();
            cmdDeleteCommand.CommandText = "truncate table BlockAlgoMinMaxSpringTest";
        ...
        private List<Node> GetDarts(SourceSinkData sourceSinkData, DateTime mDates)
        {
            ...
            try
            {
                ...
                //mDates2013.sDateTime = new DateTime(2013, 03, 01);
                //mDates2013.eDateTime = new DateTime(2013, 05, 31);
                //mDateTimeList.Add(mDates2013);

                //Dates mDates2014 = new Dates();
                //mDates2014.sDateTime = new DateTime(2014, 03, 01);
                //mDates2014.eDateTime = new DateTime(2014, 05, 31);
                //mDateTimeList.Add(mDates2014);

                //Dates mDates2015 = new Dates();
                //mDates2015.sDateTime = new DateTime(2015, 03, 01);
                //mDates2015.eDateTime = new DateTime(2015, 05, 31);
                //mDateTimeList.Add(mDates2015);

                //Dates mDates2016 = new Dates();
                //mDates2016.sDateTime = new DateTime(2016, 03, 01);
                //mDates2016.eDateTime = new DateTime(2016, 05, 31);
                //mDateTimeList.Add(mDates2016);

                //Dates mDates2017 = new Dates();
                //mDates2017.sDateTime = new DateTime(2017, 03, 01);
                //mDates2017.eDateTime = new DateTime(2017, 05, 31);
