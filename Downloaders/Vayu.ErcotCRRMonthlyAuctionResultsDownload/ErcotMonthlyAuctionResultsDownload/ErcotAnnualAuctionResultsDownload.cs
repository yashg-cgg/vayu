...
namespace Vayu.ErcotCRRMonthlyAuctionResultsDownload
{
    public class ErcotAnnualAuctionResultsDownload : ErcotMonthlyAuctionResultsDownload
    {
        ...
        public virtual void DownloadAuunalUpdate(DateTime? processDate = null)
        {
            ...
            FillPeriodHash(processDate.Value);
            for (int i = 1; i < 2; i++)
            {
        ...
        public void InsertAnnualResult(DataTable annualDT)
        {
            ...
            using (SqlBulkCopy bkAAuction = new SqlBulkCopy(DBConnection, SqlBulkCopyOptions.TableLock, transaction))
            {
                try
                {
                    bkAAuction.DestinationTableName = "[ERCOT]..[crrauctionresults_test]";
        ...
        //public void UpdateAnnualOptionPrices(string fileName, int CRRAuctionkey)
        //{
        //   // fileName = @"D:\ISOFiles\ErcotCRRMonthlyAuctionResultsDownload\ERCOTAnnualAuction\Common_AuctionBidsAndOffers_2020.2nd6.AnnualAuction.Seq1_AUCTION.xml";
        //   // FillPeriodHash(date);
        //    XmlSerializer serializer = new XmlSerializer(typeof(AuctionCRRs));
        //    AuctionCRRs mOptionPriceData;
        //    List<Crr> optionParseList = new List<Crr>();
        ...
        //    }
        //    int count1 = mOptionPriceData.Crr.Count();
        //    mOptionPriceDT.Clear();
        //    optionParseList = mOptionPriceData.Crr;
        //    var nodeName = from item in optionParseList orderby item.Source select item.Source;
        //    //List<Crr> crrHelperList = optionPriceList.Where(a => a.Source == item.Source && a.Sink == item.Sink).ToList();
        //    foreach (var item in nodeName.Distinct())
        //    {
        //        // List<ShadowPriceElement> spElement = shadowpriceList.Where(x => x.SourceSink == item.ToString()).ToList();
        //        var shadowPriceList = from spitem in optionParseList where spitem.Source == item select spitem;
        //        var mPeriodList = from pItem in shadowPriceList orderby pItem.Sink select pItem.Sink;
        //        foreach (var mPeriod in mPeriodList.Distinct())
        //        {
        //            drCRRNodal = mOptionPriceDT.NewRow();
        //            drCRRNodal["CRRAuctionKey"] = CRRAuctionkey;
        //            drCRRNodal["Sourcekey"] = NodeDictHash[item.ToString()];
        //            drCRRNodal["Sinkkey"] = NodeDictHash[item.ToString()];
        //            var spElement = from spItem in shadowPriceList where spItem.Sink == mPeriod select spItem;

        //            for (int i = 0; i < spElement.ToList().Count(); i++)
        //            {
        //                string pName = spElement.ToList()[i].Sink.ToString();
        //                string[] pArr = pName.Split('_').ToArray();
        //                if (spElement.ToList()[i].Tou == "PeakWD")
        //                {
        //                    drCRRNodal["PeakWD"] = Convert.ToDouble(spElement.ToList()[i].ShadowPrice);
        //                }
        //                if (spElement.ToList()[i].Tou == "Off-peak")
        //                {
        //                    drCRRNodal["OffPeak"] = Convert.ToDouble(spElement.ToList()[i].ShadowPrice);
        //                }                        
        //                if (spElement.ToList()[i].Tou == "PeakWE")
        //                {
        //                    drCRRNodal["PeakWE"] = Convert.ToDouble(spElement.ToList()[i].ShadowPrice);
        ...
        //                drCRRNodal["bidType"] = Convert.ToDouble(spElement.ToList()[i].ShadowPrice);
        //                drCRRNodal["HedgeType"] = Convert.ToString(spElement.ToList()[i].ShadowPrice);
        //                //drCRRNodal["startDate"] = Convert.ToDateTime(spElement.ToList()[i].ShadowPrice);
        //                //drCRRNodal["endDate"] = Convert.ToDateTime(spElement.ToList()[i].ShadowPrice);

        //                if (spElement.ToList()[i].Tou == "startDate")
        //                {
        //                   drCRRNodal["startDate"] = Convert.ToDateTime(spElement.ToList()[i].ShadowPrice);

        //                }
        //                if (spElement.ToList()[i].Tou == "endDate")
        //                {
        //                    drCRRNodal["endDate"] = Convert.ToDateTime(spElement.ToList()[i].ShadowPrice);
        //                }
        //                //drCRRNodal["PeriodKey"] = annualPeriodHash[pName];//periodHash[pArr[0]];
        //            }
        //            mOptionPriceDT.Rows.Add(drCRRNodal);
        //        }
        //    }
        //    int nodalCount = mOptionPriceDT.Rows.Count;
        //    if (mOptionPriceDT.Rows.Count > 1)
        //    {
        //        //Console.WriteLine("Option Prices Insert into DB..");
        //        InsertOptionPrice(mOptionPriceDT);
        //    }
        //}
