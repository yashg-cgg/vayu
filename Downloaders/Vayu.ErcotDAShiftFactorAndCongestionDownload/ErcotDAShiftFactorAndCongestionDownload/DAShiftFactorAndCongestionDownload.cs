...
namespace Vayu.ErcotDAShiftFactorAndCongestionDownload
{
    public class DAShiftFactorAndCongestionDownload
    {
        ...
        private void OnTimedEvent(object source, ElapsedEventArgs e)
        {
            ...
            DownloadSensitivity();
            SaveImpact(DateTime.Today.AddDays(-13), DateTime.Today.AddDays(1));
            mTimer.Enabled = true;
        }
        public void InitDB()
        {
        ...
        private void ExportToCsv(DataTable table, string filePath)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                IEnumerable<string> fields = row.ItemArray.Select(field =>
                    field.ToString().Replace(",", " ")); // avoid comma issues
        ...
        private void SaveSensitivity(DataRow[] rows)
        {
            ...
            foreach (DataRow row in rows)
            {
                ...
                //new added 24042026 h
                HashSet<string> vectorKeySet = new HashSet<string>();
                ...
                string vectorkey = constraintId + "_" + nodeKey;
                if(vectorKeySet.Contains(vectorkey))
                {
                    Console.WriteLine($"Duplicate skipped:{vectorkey}");
                    continue;
                }
                vectorKeySet.Add(vectorkey);

                mDtErcotShftFact.Rows.Add(drErcotShftFact);
                ExportToCsv(mDtErcotShftFact, @"C:\Temp\ErcotShftFact.csv");
