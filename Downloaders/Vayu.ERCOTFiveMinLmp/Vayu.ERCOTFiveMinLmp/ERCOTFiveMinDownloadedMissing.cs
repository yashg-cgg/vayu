...
{
    class ERCOTFiveMinDownloadedMissing
    {
        ...
        public ERCOTFiveMinDownloadedMissing()
        {
        ...
        private void OnTimerEvent(object source, ElapsedEventArgs e)
        {
            ...
            //DateTime today = DateTime.Today.AddDays(0);
             DateTime today = new DateTime(2024,04,09);
        ...
        private void DownloadLmp(DateTime today)
        {
            ...
            //cmd.CommandText = "select distinct MarketDate , MarketHour , MarketMin , Second from NodeLMPMin where   MarketDate = '" + today.Date.ToString() + "' order by MarketDate desc, MarketHour desc, MarketMin desc";
            ...
            //    int second = Convert.ToInt32(reader.GetValue(3));
            //    mktDate = mktDate.AddHours(hour).AddMinutes(min).AddSeconds(second);
        ...
        public void DownloadSsp(DateTime today)
        {
            ...
            while (start <= DateTime.Now)
            {
                ...
                try
                {
                    ...
                    string minStr = "02";// min < 10 ? "0" + min : min.ToString();
                    string hourStr = "23";// hour < 10 ? "0" + hour : hour.ToString();
                    bool isDST = curTimeZone.IsDaylightSavingTime(DateTime.Now);
                    DateTime endTime = isDST ? Convert.ToDateTime(today.Year + "-" + today.Month + "-" + today.Day + "T" + hourStr + ":" + minStr + ":00-05:00") :
                                        Convert.ToDateTime(today.Year + "-" + today.Month + "-" + today.Day + "T" + hourStr + ":" + minStr + ":00-06:00");
                    myRequest.EndTime = endTime.AddHours(2).AddMinutes(32);
                    myRequest.StartTime = start.AddMinutes(18).AddSeconds(6);
