...
namespace Vayu.FrozenSolarPowerGenerationvaluesDownloader
{
    public class Solarpower
    {
        ...
        public void InitDB()
        {
            ...
            dtsolarpower.Columns.Add("FreezeDate", typeof(DateTime));
            dtsolarpower.Columns.Add("FrozenHour", typeof(int));
        }
        ...
        public void InsertDB(DataTable dtSolar)
        {
            try
            {
                ...
                using (SqlBulkCopy bkSolar = new SqlBulkCopy(VayuDBConnection, SqlBulkCopyOptions.TableLock, transaction))
                {
                    try
                    {
                        bkSolar.DestinationTableName = "Vayu..FrozenSolarPowerGenerationValue_test";
                        ...
                        bkSolar.ColumnMappings.Add("FreezeDate", "FreezeDate");
                        bkSolar.ColumnMappings.Add("FrozenHour", "FrozenHour");
                        bkSolar.WriteToServer(dtSolar);
                        SqlCommand UpdateSolarPower = new SqlCommand("[dbo].[UpMergeFrozenSolarPower]", VayuDBConnection, transaction);
        ...
        public void DownloadSolarpower()
        {
            try
            {
                ...
                strArrayList=strArrayList.Where(x => x.Contains("09:55")).ToArray();
               
                //for (int p = 0; p <= strArrayList.Length; p++)
                {
                    string dataXml = strArrayList[1];
                    if (dataXml.Contains("PVGRHRLYAVGACTNP4737_xml.zip"))
                    {
                        int publishdate = dataXml.IndexOf("PublishDate");
                        string pDate = dataXml.Substring(publishdate + 14, 19);
                        pDate = pDate.Replace("-", "");
                        pDate = pDate.Replace(":", "");
                        ...
                        //if (filedate1 == DateTime.Today)
                        {
                            ...
                            if (headers != null)
                            {
                                ...
                                string fileDestName = @"D:\\ISOFiles\\ErcotSolarPowerGenerationvaluesDownloader\\FrozenSolarPower\\" + name;
                                ...
                                int hour; DateTime datetime;
                                string NewFileName = XML_UnZipFile(fileDestName, @"D:\ISOFiles\ErcotSolarPowerGenerationvaluesDownloader\FrozenSolarPower\");
                                XmlSerializer serializer = new XmlSerializer(typeof(SolarPowerProductionHourlyAverageActualForecastedValues));
                                SolarPowerProductionHourlyAverageActualForecastedValues SolarPowerProductionHourly;
                                using (XmlReader reader = XmlReader.Create(NewFileName))
                                {
                                ...
                                foreach (var item in SolarPowerProductionHourly.SolarPowerProductionHourlyAverageActualForecastedValue)
                                {
                                    ...
                                    if (datetime.Date >= publishedDate.Date.AddDays(1))
                                    {
                                        ...
                                        drsolarpower["PVGRPPvalue"] = Convert.ToDecimal(item.PVGRPP_SYSTEM_WIDE);
                                        drsolarpower["DSTFlag"] = Convert.ToString(item.DSTFlag);
                                        drsolarpower["FreezeDate"] = publishedDate;
                                        drsolarpower["FrozenHour"] = 9;
                                        dtsolarpower.Rows.Add(drsolarpower);
                                    }
                                }
