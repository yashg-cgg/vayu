...
namespace Vayu.NodeLMPDailyImport
{
    ...
    {
        private static string LMPServiceEndPoint =ServiceConnections.GetLMPServiceAddress();
        ...
        public void InitDB()
        {
            ...
            //
            mNodeLMPErcotDT.Columns.Add("NodeKey", typeof(int));
            ...
            mDeleteCaisoLMPDailyCommand.CommandText = "Truncate table [NewTrading].[Caiso].[NodeLMPDaily_test]";
        }
        public void Run(DateTime startDate, DateTime endDate, int marketKey)
        {
            try
            {
                ...
                ChannelFactory<ILMP> pipeFactory = new ChannelFactory<ILMP>(myBinding, new EndpointAddress(LMPServiceEndPoint));
        ...
        private void InsertLmps(int marketKey, ILMP nodeProxy, DateTime finalstartdate, DateTime finalendtdate, Node[] daNodes, Node[] rtNodes)
        {
            ...
            if (marketKey == 9)
            {
                ...
                SaveLmpRTDAERCOT(rtNodes, "RT", finalstartdate, finalendtdate, marketKey);
            }
        }
        ...
        private void SaveLmpRTDAERCOT(Node[] nodeHash, string MarketTypeCode, DateTime finalStartDate, DateTime finalEndDate, int marketKey)
        {
            try
            {
                ...
                foreach (Node priceNode in nodeHash)
                {
                    ...
                    SetDailyAverageErcot(marketKey, priceNode, peakList, offpeakList, peakweList,nodeLmpRow, finalStartDate, false, MarketTypeCode);
                ...
                mDeleteNYISONodeLMPDailyCommand.CommandText = "Truncate table Vayu..NodeLMPDaily_Test";
                ...
                using (SqlBulkCopy bkLmpH = new SqlBulkCopy(DBConnection, SqlBulkCopyOptions.TableLock, transaction))
                {
                    try
                    {
                        ...
                        bkLmpH.DestinationTableName = "Vayu..NodeLMPDaily_Test";
                        ...
                        SqlCommand updatenodelmph = new SqlCommand("DeleteDupsDailyLMPImps", DBConnection, transaction);
                ...
                using (SqlBulkCopy bkLmpH = new SqlBulkCopy(DBConnection, SqlBulkCopyOptions.TableLock, transaction1))
                {
                    try
                    {
                        ...
                        SqlCommand updatenodelmph = new SqlCommand("UpMergeNodeLMPDailys", DBConnection, transaction1);
                        updatenodelmph.CommandType = CommandType.StoredProcedure;
                        updatenodelmph.CommandTimeout = 30000;
                        var upcount = updatenodelmph.ExecuteNonQuery();
                        transaction1.Commit();
        ...
        private void SaveLmpRTDA(Node[] nodeHash, string MarketTypeCode, DateTime finalStartDate, DateTime finalEndDate, int marketKey)
        {
            try
            {
                ...
                mDeleteNYISONodeLMPDailyCommand = new SqlCommand();
                mDeleteNYISONodeLMPDailyCommand.CommandText = "Truncate table Vayu..NodeLMPDailyTest";
                mDeleteNYISONodeLMPDailyCommand.Connection = DBConnection;
                mDeleteNYISONodeLMPDailyCommand.ExecuteNonQuery();
                SqlTransaction transaction = DBConnection.BeginTransaction();
                using (SqlBulkCopy bkLmpH = new SqlBulkCopy(DBConnection, SqlBulkCopyOptions.TableLock, transaction))
                {
                    try
                    {
                        ...
                        bkLmpH.DestinationTableName = "Vayu..[NodeLMPDailyTest]";
                        bkLmpH.ColumnMappings.Add("NodeKey", "NodeKey");
                        bkLmpH.ColumnMappings.Add("MarketTypeCode", "MarketTypeCode");
                        bkLmpH.ColumnMappings.Add("MarketDate", "MarketDate");
                        bkLmpH.ColumnMappings.Add("AvgPeakLMP", "AvgPeakLMP");
                        ...
                        SqlCommand updatenodelmph = new SqlCommand("DeleteDupsDailyLMPImp", DBConnection, transaction);
                        updatenodelmph.CommandType = CommandType.StoredProcedure;
                        updatenodelmph.CommandTimeout = 30000;
                        var upcount = updatenodelmph.ExecuteNonQuery();
                        transaction.Commit();
                ...
                using (SqlBulkCopy bkLmpH = new SqlBulkCopy(DBConnection, SqlBulkCopyOptions.TableLock, transaction1))
                {
                    try
                    {
                        ...
                        SqlCommand updatenodelmph = new SqlCommand("UpMergeNodeLMPDaily", DBConnection, transaction1);
                        updatenodelmph.CommandType = CommandType.StoredProcedure;
                        updatenodelmph.CommandTimeout = 30000;
                        var upcount = updatenodelmph.ExecuteNonQuery();
                        transaction1.Commit();
            ...
            try
            {
                foreach (LmpTimePrice lmpTimePrice in priceNode.LmpTimePriceList)
                {
                    if (lmpTimePrice.Lmp == null)
                        continue;
                    if (Convert.ToInt32(priceNode.NodeId) == 2805)
                    {
        ...
        private void SetDailyAverageErcot(int marketKey, Node priceNode, List<DateTime> peakList, List<DateTime> offpeakList, List<DateTime> peakweList, DataRow nodeLmpRow, DateTime finalStartDate, bool value, string MarketTypeCode1)
        {
            double? avgPeakLmp = null;
            double? avgPeakLoss = null;
            double? avgPeakCong = null;
                ...
                foreach (LmpTimePrice lmpTimePrice in priceNode.LmpTimePriceList)
                {
                    ...
                    if ((!double.IsNaN(lmpTimePrice.Lmp.Price)) || (!double.IsNaN(lmpTimePrice.Lmp.Congestion)) || (!double.IsNaN(lmpTimePrice.Lmp.Loss)))
                    {
                        ...
                        else if (peakweList.Contains(lmpTimePrice.MarketTime))
                        {
                            ...
                            {
                                avgPeakWELmp = Convert.ToDouble(lmpTimePrice.Lmp.Price);
                            }
                            else
                            {
                                avgPeakWELmp += Convert.ToDouble(lmpTimePrice.Lmp.Price);
