...
namespace Vayu.CRRPNLDownload
{
    ...
    class CRRPNLdownloadISO
    {
        ...
        private SqlCommand mDeleteSPPPositionPnlCommand;
        private SqlCommand mDeleteMISOPositionPnlCommand;
        private SqlCommand mDeletePJMPositionPnlCommand;
        private SqlCommand mDeleteCAISOPositionPnlCommand;
        ...
        private SqlCommand mSelectSPPParticipantsCommand;
        private SqlCommand mSelectCAISOMarketParticipantsCommand;
        private DataTable mPositionPnlDT = new DataTable();
        private string mEndPoint = CommonAccessLibrary.ServiceConnections.GetFTRService();
        private ISourceSink mFTRCalculationProxy = null;
        // DateTime mstartDate = new DateTime(DateTime.Today.Year, DateTime.Today.Month - 1, 01);
        DateTime mstartDate = new DateTime(2022,01, 01);
        ...
        private void InitDB()
        {
            ...
            //
            mDeleteSPPPositionPnlCommand = new SqlCommand();
            mDeleteSPPPositionPnlCommand.CommandText = "delete SPP.FTRPostionPNL where FTRID in (select distinct FTRID from SPP.FTRAuctionResults where Participant =@ftrid) and  date between @start and @end ";
            mDeleteSPPPositionPnlCommand.Parameters.AddWithValue("@ftrid", "ftrid");
            mDeleteSPPPositionPnlCommand.Parameters.AddWithValue("@start", "date");
            mDeleteSPPPositionPnlCommand.Parameters.AddWithValue("@end", "date");
            mDeleteSPPPositionPnlCommand.Connection = mConnection90;
            //
            mDeletePJMPositionPnlCommand = mConnection90.CreateCommand();
            mDeletePJMPositionPnlCommand.CommandText = "delete from pjm.FTRPostionPNL where FTRID in (select distinct FTRID from pjm.FTRAuctionResults where Participant =@ftrid) and date between @start and @end ";
            mDeletePJMPositionPnlCommand.Parameters.AddWithValue("@ftrid", "ftrid");
            mDeletePJMPositionPnlCommand.Parameters.AddWithValue("@start", "date");
            mDeletePJMPositionPnlCommand.Parameters.AddWithValue("@end", "date");
            //
            mDeleteMISOPositionPnlCommand = new SqlCommand();
            mDeleteMISOPositionPnlCommand.CommandText = "delete MISO.FTRPostionPNL where FTRID in (select distinct FTRID from MISO.FTRAuctionResults where Participant =@ftrid) and  date between @start and @end ";
            mDeleteMISOPositionPnlCommand.Parameters.AddWithValue("@ftrid", "ftrid");
            mDeleteMISOPositionPnlCommand.Parameters.AddWithValue("@start", "date");
            mDeleteMISOPositionPnlCommand.Parameters.AddWithValue("@end", "date");
            mDeleteMISOPositionPnlCommand.Connection = mConnection90;
            //
            mDeleteCAISOPositionPnlCommand = new SqlCommand();
            mDeleteCAISOPositionPnlCommand.CommandText = "delete CAISO.FTRPostionPNL where  FTRID in (select distinct FTRID from CAISO.FTRAuctionResults where Participant =@ftrid) and  date between @start and @end ";
            mDeleteCAISOPositionPnlCommand.Parameters.AddWithValue("@ftrid", "ftrid");
            mDeleteCAISOPositionPnlCommand.Parameters.AddWithValue("@start", "date");
            mDeleteCAISOPositionPnlCommand.Parameters.AddWithValue("@end", "date");
            mDeleteCAISOPositionPnlCommand.Connection = mConnection90;
            //
            ...
            mSelectERCOTParicipantsCommand.CommandText = "select distinct(AccountHolder) from CRRAuctionResults  where  AccountHolder like '%XISO2%' ";
            mSelectERCOTParicipantsCommand.Connection = mConnection90;
            //
            mSelectSPPParticipantsCommand = new SqlCommand();
            mSelectSPPParticipantsCommand.CommandText = "select distinct(Participant) from SPP.FTRAuctionResults order by participant";
            mSelectSPPParticipantsCommand.Connection = mConnection90;
            //
            mSelectCAISOMarketParticipantsCommand = new SqlCommand();
            mSelectCAISOMarketParticipantsCommand.CommandText = "select distinct Participant from CAISO.FTRAuctionResults order by participant";
            mSelectCAISOMarketParticipantsCommand.Connection = mConnection90;
        }
        ...
        private void DownloadCRRPNL()
        {
            ...
            {
                startDate = new DateTime(2022, 01, 01);//DateTime(endDate.AddMonths(0).Year, endDate.AddMonths(0).Month-1, 1);//DateTime.Parse(endDate.AddMonths(-3).Month + "/1/" + endDate.AddMonths(-3).Year);
            }
            PrepareAllAccounts();
#if PRODUCTION
            Parallel.For(0, 3, k =>
        ...
        private Dictionary<string, Tuple<double, Dictionary<DateTime, DailyPnlHelper>>> GetFormattedSourceSink(List<SourceSink> sourcesinks)
        {
            ...
            Dictionary<string, double> mwHashoffpeak = new Dictionary<string, double>();
            Dictionary<string, double> mwHashpeak = new Dictionary<string, double>();
            Dictionary<string, double> mwHashpeakwe = new Dictionary<string, double>();
            PeriodDays days = GetPeriodDays(mstartDate, mstartDate.AddMonths(1).AddDays(-1), 9);
            ...
            foreach (SourceSink item in sourcesinks)
            {
                try
                {
                    double comparemwoffpeak = 0;
                    double comparemwpeak = 0;
                    double comparemwpeakwe = 0;
                    if (item.ClassType == "Off-peak" || item.ClassType == "OFF - PEAK" || item.ClassType == "OFFPEAK" || item.ClassType == "Off-Peak")
                    {
                        comparemwoffpeak = Math.Round(item.MW, 2) / (days.OffPeakDays*0.33);
                        if (mwHashoffpeak.ContainsKey(item.Participant))
                        {
                            mwHashoffpeak[item.Participant] += comparemwoffpeak;
                        }
                        else
                        {
                            mwHashoffpeak.Add(item.Participant, comparemwoffpeak);
                        }
                    }
                    else if (item.ClassType.ToUpper() == "PEAK" || item.ClassType.ToUpper() == "ONPEAK" || item.ClassType.ToUpper() == "PEAKWD")
                    {
                        comparemwpeak = Math.Round(item.MW, 2) / (days.PeakDays*0.66);
                        if (mwHashpeak.ContainsKey(item.Participant))
                        {
                            mwHashpeak[item.Participant] += comparemwpeak;
                        }
                        else
                        {
                            mwHashpeak.Add(item.Participant, comparemwpeak);
                        }
                    }
                    else if (item.ClassType.ToUpper() == "PEAKWE")
                    {
                        comparemwpeakwe = Math.Round(item.MW, 2) / days.PeakWEDays*0.66+ Math.Round(item.MW, 2) / (days.OffPeakDays * 0.33);
                        if (mwHashpeakwe.ContainsKey(item.Participant))
                        {
                            mwHashpeakwe[item.Participant] += comparemwpeakwe;
                        }
                        else
                        {
                            mwHashpeakwe.Add(item.Participant, comparemwpeakwe);
                        }
                    }
                    ...
                    foreach (var innerItem in item.dailyPNL)
                    {
                        ...
                        if (item.ClassType == "Off-peak" || item.ClassType == "OFF - PEAK" || item.ClassType == "OFFPEAK" || item.ClassType == "Off-Peak")
                        {
                            if (innerItem.Key.DayOfWeek == DayOfWeek.Monday || innerItem.Key.DayOfWeek == DayOfWeek.Tuesday || innerItem.Key.DayOfWeek == DayOfWeek.Wednesday || innerItem.Key.DayOfWeek == DayOfWeek.Thursday || innerItem.Key.DayOfWeek == DayOfWeek.Friday)
                            {
                                if (holidayList.Contains(innerItem.Key))
                                {
                                    helperTemp.MW = mwHashpeakwe.ContainsKey(item.Participant) ? mwHashpeakwe[item.Participant] : 0;
                                }
                                else
                                {
                                    helperTemp.MW = mwHashoffpeak.ContainsKey(item.Participant) ? mwHashoffpeak[item.Participant] : 0;
                                }
                            }
                        }
                        else if ( item.ClassType.ToUpper() == "PEAK" || item.ClassType.ToUpper() == "ONPEAK" || item.ClassType.ToUpper() == "PEAKWD" )
                        {
                            if (innerItem.Key.DayOfWeek == DayOfWeek.Monday || innerItem.Key.DayOfWeek == DayOfWeek.Tuesday || innerItem.Key.DayOfWeek == DayOfWeek.Wednesday || innerItem.Key.DayOfWeek == DayOfWeek.Thursday || innerItem.Key.DayOfWeek == DayOfWeek.Friday)
                            {
                                ...
                                {
                                    helperTemp.MW = mwHashpeak.ContainsKey(item.Participant) ? mwHashpeak[item.Participant] : 0;
                                }
                            }
                        }
                        else if (item.ClassType.ToUpper() == "PEAKWE")
                        {
                            if (innerItem.Key.DayOfWeek == DayOfWeek.Saturday || innerItem.Key.DayOfWeek == DayOfWeek.Sunday)
                            {
                                if (holidayList.Contains(innerItem.Key))
                                {
                                    helperTemp.MW = mwHashpeakwe.ContainsKey(item.Participant) ? mwHashpeakwe[item.Participant] : 0;
                    ...
                    double mw = (mwHashpeakwe.ContainsKey(item.Participant) ? mwHashpeakwe[item.Participant] : 0) + (mwHashoffpeak.ContainsKey(item.Participant) ? mwHashoffpeak[item.Participant] : 0) + (mwHashpeak.ContainsKey(item.Participant) ? mwHashpeak[item.Participant] : 0);
            ...
            try
            {
                ...
                foreach (var item in dailyHash)
                {
                    ...
                    foreach (var innerItem in item.Value.Item2)
                    {
                        ...
                        dr[2] = Math.Round(innerItem.Value.MW, 2);
        ...
        public PeriodDays GetPeriodDays(DateTime startDate, DateTime endDate, int? Key)
        {
            PeriodDays periodHours = new PeriodDays();
            if (Key == 9)
            {
                using (SqlConnection con = new VayuDBConnection().GetInstance().GetSqlConnection())
                {
                    using (SqlCommand cmd = con.CreateCommand())
                    {
                        cmd.CommandText = " select PeakDays , OffPeakDays, peakwedays from Period where PeriodType = 'Monthly' and StartDate = '" + startDate.ToString() + "' and EndDate = '" + endDate.ToString() + "'";
                        cmd.Connection = con;
                        con.Open();
                        SqlDataReader rdr = cmd.ExecuteReader();
                        while (rdr.Read())
                        {
                            periodHours.PeakDays = rdr.IsDBNull(0) ? 0 : Convert.ToInt32(rdr.GetValue(0));
                            periodHours.OffPeakDays = rdr.IsDBNull(1) ? 0 : Convert.ToInt32(rdr.GetValue(1));
                            periodHours.PeakWEDays = rdr.IsDBNull(2) ? 0 : Convert.ToInt32(rdr.GetValue(2));
                        }
                        rdr.Close();
                        con.Close();
                    }
        ...
        public List<DateTime> GetHolidayList()
        {
            ...
            using (SqlConnection con = new VayuDBConnection().GetInstance().GetSqlConnection())
            {
                using (SqlCommand cmd = con.CreateCommand())
                {
                    cmd.CommandText = " select Date from Holiday ";
                    cmd.Connection = con;
                    con.Open();
                    SqlDataReader rdr = cmd.ExecuteReader();
                    while (rdr.Read())
    ...
    public class PeriodDays
    {
        ...
        public int OffPeakDays { get; set; }
        public int PeakWEDays { get; set; }
    }
}
