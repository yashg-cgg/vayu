...
namespace Vayu.PNLDownload
{
    ...
    public class VirtualPnlDownload
    {
       ...
       // private DateTime mStartDate = new DateTime(2023, 05, 03);
        /// <summary>
        /// The realtime hash
        /// </summary>
        ...
        public void GetUptosPnl(string iso)
        {
            InitDB();
            if (iso == "PJM")
                pnlTableName = "virtual_pnl";
            else if (iso == "ERCOT")
                pnlTableName = "Vayu..DailyPnl";
            try
            {
                ...
                while (startDate <= endDate)
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (iso == "ERCOT")
                        {
                            mSelectPortfolioDetailCommand.CommandText = "select marketDateTime, sourcenodekey, sinknodekey, clearedmw from vayu..ClearedEES where MarketDateTime > @startDate and MarketDateTime <= @endDate and portfolioKey = @portfolioKey and PortfolioKey in(select distinct PORTFOLIO_ID from PORTFOLIO where ACCOUNT in(select account_id from account where  account_id in(select distinct account from portfolio where product='EES/PTP' and hub='ERCOT')  ))";
                        }
        ...
        ///<summary>
        ///Gets the External PNL
        ///</summary>

        public void GetExternalPnl(string iso)
        {
            InitDB();
            if (iso == "ERCOT External")
                pnlTableName = "Vayu..ExternalPnl";
            ...
            try
            {
                ...
                DateTime startDate = mStartDate;
                //  DateTime endDate = DateTime.Parse(DateTime.Today.AddDays(-1).ToShortDateString());
                DateTime endDate = DateTime.Parse(DateTime.Today.AddMonths(-1).ToShortDateString());
                ...
                while (startDate <= endDate)
                {
                    ...
                    if (iso == "ERCOT External")
                    {
                       // mSelectPortfolioCommand.CommandText = "select distinct portfoliokey from ercot..ClearedEES where MarketDateTime > @startDate and MarketDateTime <= @endDate and PortfolioKey in(select distinct PORTFOLIO_ID from PORTFOLIO where ACCOUNT in(select account_id from account where  account_id in(select distinct account from portfolio where product='EES/PTP' and hub='ERCOT')  ))";
                        mSelectPortfolioCommand.CommandText = "select distinct ParticipantID from Vayu..DAM60DAYPTPAWARDS where DeliveryDate > @startDate and DeliveryDate <= @endDate";
                    }
                    mSelectPortfolioCommand.Parameters["@startDate"].Value = startDate;
                    DateTime endEndDate = startDate.AddDays(1);
                    mSelectPortfolioCommand.Parameters["@endDate"].Value = endEndDate;
                    SqlDataReader reader = mSelectPortfolioCommand.ExecuteReader();
                    while (reader.Read())
                    {
                        double totalPnl = 0;
                        ...
                        if (iso == "ERCOT External")
                        {
                           // mSelectPortfolioDetailCommand.CommandText = "select marketDateTime, sourcenodekey, sinknodekey, clearedmw from ercot..ClearedEES where MarketDateTime > @startDate and MarketDateTime <= @endDate and portfolioKey = @portfolioKey and PortfolioKey in(select distinct PORTFOLIO_ID from PORTFOLIO where ACCOUNT in(select account_id from account where  account_id in(select distinct account from portfolio where product='EES/PTP' and hub='ERCOT')  ))";
                            mSelectPortfolioDetailCommand.CommandText = "select DeliveryDate, SourceKey, SinkKey, MW from Vayu..DAM60DAYPTPAWARDS where DeliveryDate > @startDate and DeliveryDate <= @endDate and ParticipantID = @portfolioKey";
                        }
                        mSelectPortfolioDetailCommand.Parameters["@startDate"].Value = startDate;
                        mSelectPortfolioDetailCommand.Parameters["@endDate"].Value = endEndDate;
                        mSelectPortfolioDetailCommand.Parameters["@portfolioKey"].Value = portfolioId;
                        SqlDataReader reader1 = mSelectPortfolioDetailCommand.ExecuteReader();
                        DataTable dt = new DataTable();
                        dt.Load(reader1);
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            totalmw += mw;
                            totalPnl += CalculatePnl(marketDate, sourceKey, sinkKey, mw);
                            Logeriter.writeLog("calculating External pnl " + totalPnl.ToString() + "\t date" + marketDate + "\t MW" + mw);
                        }
                        try
                        {
                            if (totalPnl != 0)
                            {
                                ...
                                {
                                   // mInsertVirtaulPnlCommand.CommandText = "insert into " + pnlTableName + " values (@portfoliokey, @pnldate, @pnl, 0, @pnlUpdateTime, null, @MW, @DollarCleared)";
                                    mInsertVirtaulPnlCommand.CommandText = "insert into " + pnlTableName + " values (@portfoliokey, @pnldate, @pnl, @pnlUpdateTime, @MW, @DollarCleared)";
                                    mInsertVirtaulPnlCommand.Connection = VayuDbConnection;
                                    mInsertVirtaulPnlCommand.Parameters["@portfoliokey"].Value = portfolioId;
                                    mInsertVirtaulPnlCommand.Parameters["@pnldate"].Value = startDate;
                                    mInsertVirtaulPnlCommand.Parameters["@pnl"].Value = totalPnl;
                                ...
                                Logeriter.writeLog("inserting External pnl" + "portfoliokey" + portfolioId + "\tStartDate" + startDate + "\t totalPnl" + totalPnl);
                                Console.WriteLine("Inserting External pnl");
                            }
                        }
                        catch (Exception ex)
                        {
