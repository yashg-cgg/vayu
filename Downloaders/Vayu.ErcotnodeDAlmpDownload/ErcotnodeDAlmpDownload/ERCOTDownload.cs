    ...
    public class ERCOTNodeDALmpDownload
    {
        ...
        private void FillNodeHash()
        {
            ...
            while (reader.Read())
            {
                ...
                if (!mNodeHash.ContainsKey(reader.GetString(1)))
                {
        ...
        public void InitDB()
        {
            ...
            mInsertNodeCommand.CommandText = "if not exists (select * from Vayu..node where nodename=@nodename and marketkey=9 )" +
                                                    "insert ercot..node values (@nodename,  0, 9, 32, 'SOUTH' , NULL , NULL)";
                ...
                for (int k = 6; k < splinelitRow.Length; k = k + 6)
                {
                    ...
                    using (SqlBulkCopy bkLmpH = new SqlBulkCopy(VayuDbConn, SqlBulkCopyOptions.TableLock, transaction))
                    {
                        try
                        {
                            ...
                            SqlCommand updateNodeLmpMin = new SqlCommand("[UpMergeNodeDALMPHErcot]", VayuDbConn, transaction);
        ...
        public void DownloadLoadResourcesHist()
        {
            ...
            for (int i = 0; i < filePaths.Length; i++)
            {
                ...
                //using (SqlBulkCopy bkLmpH = new SqlBulkCopy(SigmaDbConnErcot, SqlBulkCopyOptions.TableLock, transaction))
                //{
                //    try
                //    {
                ...
                //        //  bkLmpH.ColumnMappings.Add("FinalYN", "FinalYN");
                //        bkLmpH.WriteToServer(mERCOTLoadResourcesDT);
                //        SqlCommand updateNodeLmpMin = new SqlCommand("[UpMergeNodeLMPH_History]", SigmaDbConnErcot, transaction);
                //        updateNodeLmpMin.CommandType = CommandType.StoredProcedure;
                //        updateNodeLmpMin.ExecuteNonQuery();
                //        transaction.Commit();
                //    }
