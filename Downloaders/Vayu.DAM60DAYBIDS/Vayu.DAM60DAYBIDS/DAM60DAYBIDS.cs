...
namespace Vayu.DAM60DAYBIDS
{
    class DAM60DAYBIDS
    {
        ...
        public void InitDB()
        {
            ...
            stDAMDataTable.Columns.Add("MultiHour", typeof(string));//6
        ...
        private void DownloadAwards(DateTime date)
        {
            try
            {
                string[] filePaths = Directory.GetFiles(@"D:\ISOFiles\DAM60DAYBIDS\");
                try
                {
               ...
               // DateTime date = DateTime.Today.AddDays(-1);
                ...
                for (int j = 0; j < splinelitRow.Length; j=j+3)
                {
                    if (splinelitRow[j].Contains(strDate))
                    {
                        ...
                        string FileName = "D:\\ISOFiles\\DAM60DAYBIDS\\Ercot_" + FileURL.Substring(71, 9) + ".zip";
                        ...
                        string folderFilename = @"D:\ISOFiles\DAM60DAYBIDS\";
        ...
        private void InsertDB(DataTable  DAMTable)
        {
            ...
            using(SqlBulkCopy bkDam = new SqlBulkCopy( VayuDbConn, SqlBulkCopyOptions.TableLock, transaction))
            {
                try
                {
                    ...
                    bkDam.DestinationTableName= "dbo.DAM60DAYPTPBIDS_Test";
                    ...
                    bkDam.ColumnMappings.Add("MultiHour", "MultiHour");
            ...
            try
            {
                ...
                using (SqlBulkCopy bkData = new SqlBulkCopy( VayuDbConn, SqlBulkCopyOptions.TableLock, Upmerge))
                {
                    try
                    {
                        ...
                        {
                            SqlCommand mUpmarge = new SqlCommand("[UpMergeDAM60DayPTPBIDS]",  VayuDbConn, Upmerge);
            ...
            try
            {
                if (File.Exists(InputPathOfZipFile))
                {
                    ...
                    using (ZipInputStream ZipStream = new ZipInputStream(File.OpenRead(InputPathOfZipFile)))
                    {
                        ...
                        {
                            if (theEntry.Name.Contains("60d_DAM_PTPObligationBids"))
                            {
                                if (theEntry.IsFile)
                                {
                                    if (theEntry.Name != "")
