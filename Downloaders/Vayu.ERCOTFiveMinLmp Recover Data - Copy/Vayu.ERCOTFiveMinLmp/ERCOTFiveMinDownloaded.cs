        ...
        public void DownloadSsp(DateTime today)
        {
            ...
            try
            {
                ...
                for (int i = 3; i < 66; i = i + 6)
                {
                        ...
                        using (StreamReader streamreader = new StreamReader(csvFileName))
                        {
                            ...
                            while (!streamreader.EndOfStream)
                            {
                                ...
                                if (values.Length >= 4)
                                {
                                    ...
                                    int hour = Int32.Parse(values[1]);
                                    int min = 0;
                                    int minvalue= Int32.Parse(values[2]);
                                    if(hour==24 && minvalue == 4)
                                    {
                                        dateTime = dtMarketDate.AddDays(1);
                                    }
                                    else
                                    {
                                        if (minvalue != 4)
                                        {
                                            min = 15 * minvalue;
                                            dateTime = dtMarketDate.AddHours(hour - 1).AddMinutes(min);
                                        }
                                        else
                                        {
                                            dateTime = dtMarketDate.AddHours(hour);

                                        }
                                    }
                                    printMarketDate = dateTime;
                                    row["MarketDateTime"] = dateTime;
                                    row["Lmp"] = decimal.Parse(values[5]);
                                    row["Congestion"] = 0;
                                    row["Loss"] = 0;
                                    row["FinalYN"] = "Y";
                                    string nodename = values[3].ToString();
                    ...
                    CalculateHourly(printMarketDate);
                }
            }
            catch (Exception ex)
            {
