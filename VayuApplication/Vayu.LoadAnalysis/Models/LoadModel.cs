...
namespace Vayu.LoadAnalysis.Models
{
    /// <summary>
    /// 
    /// </summary>
    /// <seealso cref="Vayu.LoadAnalysis.Models.IDataService" />
    public class LoadModel : IDataService
    {
        ...
        /// <param name="endDate">The end date.</param>
        /// <param name="marketKey">The market key.</param>
        public void GetConstraintContingencyData(Action<List<ConstraintContingency>, Exception> callback, DateTime startDate, DateTime endDate, int marketKey)
        {
            List<ConstraintContingency> ccdata = new List<ConstraintContingency>();
            ...
            try
            {
                if (marketKey == 1)
                {
                    cmd.CommandText = "Select distinct constrainttext, contingencytext, IsNull(ShadowPrice,0) ShadowPrice from PJM.constraintrt " +
                ...
                {
                    cmd.CommandText = "Select distinct ConstraintName,ContingencyName,IsNull(ShadowPrice,0) ShadowPrice from MISO.constraintrt " +
                                                "Where marketdatetime >= @startDate And marketdatetime < @endDate " +
                                                "And ConstraintName <> 'none' ";
                }
                else if (marketKey == 7)
                {
                    cmd.CommandText = "Select distinct ConstraintName,ContingencyName,IsNull(ShadowPrice,0) ShadowPrice from CAISO.constraintrt " +
                                                "Where marketdatetime >= @startDate And marketdatetime < @endDate " +
                                                "And ConstraintName <> 'none' ";
                }
                else
                {
                    cmd.CommandText = "Select distinct ConstraintName,ContingentFacility,IsNull(ShadowPrice,0) ShadowPrice from SPP.constraintrt " +
                                                "Where marketdatetime >= @startDate And marketdatetime < @endDate " +
                                                "And ConstraintName <> 'none' ";
                }
                ...
                cmd.Parameters["@startDate"].Value = startDate.AddHours(-1);
                cmd.Parameters["@endDate"].Value = endDate.AddHours(-1);
                ...
                while (reader.Read())
                {
                    ConstraintContingency data = new ConstraintContingency();
                    data.Constraint = reader[0].ToString();
                    data.Contingency = reader[1].ToString();
