...
namespace Vayu.ErcotShiftFactor.Model
{
    ...
    {
        /// <summary>
        /// The SigmaDbConn database connection
        /// </summary>
        ...
        SqlCommand mSelectERCOTDConstraintCmd;

        SqlCommand mSelectERCOTDAContingencyCmd;
        SqlCommand mSelectERCOTContingencyCmd;
        SqlCommand mSelectERCOTContingenciesCmd;
        ...
        private Dictionary<string, List<ErcotEnergyPriceHelper>> AllhourMinListDicEP = null;//=new Dictionary<string, List<ErcotEnergyPriceHelper>>();
        ...
        public List<Constraint> GetEneryPrice(DateTime fromDate, DateTime toDate, bool isDA)
        {
            ...
            if (isDA)
            {
                sqlCommandEnergyPrice.CommandText = " select  DeliveryDate, DAY(DeliveryDate),FORMAT(DeliveryDate,'HH'),FORMAT(DeliveryDate,'mm'),FORMAT(DeliveryDate,'ss'),SystemLambda from Ercot..DAMSystemLambda where  " +
            ...
            else
            {
                ...
                sqlCommandEnergyPrice.CommandText = " select  DeliveryDate, DAY(DeliveryDate),FORMAT(DeliveryDate,'HH'),FORMAT(DeliveryDate,'mm'),FORMAT(DeliveryDate,'ss'),SystemLambda from Ercot..RTSystemLambda where  " +
                                                    "DeliveryDate>=@Startdate and DeliveryDate <=@endDate order by DeliveryDate desc";
            }
            ...
            // foreach (var value in DatehourList)
            {
        ...
        List<string> getMissingPrints(DateTime DateVal, int Hourval, int MinVal)
        {
            ...
            try
            {
                ...
                mSelectSCEDPrintCommond.CommandText = "Select distinct MarketMin, Second from ERCOT..NodeLMPMin where MarketDate = @DateVal and  Markethour=@Hourval and MarketMin > @MinVal";
                    ...
                    foreach (DateTime date in datetimeList)
                    {
                        ...
                        if (!loadDictHash.ContainsKey(date.AddDays(-1).Date.ToString("dd-MM-yyyy")))
                        {
                            ...
                            else //For Ercot
                            {
                                ...
                                mSelectloadDailyCommand.CommandText = "select CAST(MarketDateTime as date) date , max(mw) from ERCOT..LoadRT where loadskey=2213 and   MarketDateTime >= @Startdate and MarketDateTime < @endDate"
                                                                      + " group by CAST(MarketDateTime as date) order by date";
                                mSelectloadDailyCommand.Parameters.AddWithValue("@Startdate", date.Date.AddDays(-1));
                                mSelectloadDailyCommand.Parameters.AddWithValue("@endDate", date.Date);
                                mSelectloadDailyCommand.Connection = VayuConnection;
                                ...
                                {
                                    mSelectloadHourlyCommand = new SqlCommand();
                                    mSelectloadHourlyCommand.CommandText = "select  max(MW) from ERCOT..LoadRT where LoadsKey = 2213 and MarketDateTime >=  @StartDate and MarketDateTime < @EndDate";//LoadRTH 
                                    mSelectloadHourlyCommand.Parameters.AddWithValue("@StartDate", date.Date.AddDays(-1));
                                    mSelectloadHourlyCommand.Parameters.AddWithValue("@EndDate", date.Date);
                                    mSelectloadHourlyCommand.Connection = VayuConnection;
                                    SqlDataReader reader1 = mSelectloadHourlyCommand.ExecuteReader();
                ...
                using (SqlConnection con = new VayuDBConnection().GetInstance().GetSqlConnection())
                {
                    using (SqlCommand cmd = con.CreateCommand())
                    {
                        ...
                        cmd.CommandText = "select a.ConstraintRTNum , b.MonitoredText , b.ContingencyText , c.NodeName , a.Sensitivity , a.source , a.nodekey , c.Zone  " +
                                             ...
                                             "where b.MonitoredText = '" + constraintName + "' and b.ContingencyText = '" + contingencyName + "' UNION " +
                                         "select a.ConstraintRTNum , b.MonitoredText , b.ContingencyText , c.NodeName , a.Sensitivity , a.source , a.nodekey , c.Zone " +
        ...
        /// <summary>
        /// Fills the average hash by service.
        /// </summary>
        /// <param name="marketKey">The market key.</param>
        /// <param name="isDa">if set to <c>true</c> [is da].</param>
        /// <param name="fromDate">From date.</param>
            ...
            foreach (var item in constraintList)
            {
                try
                {
                    ...
                    // if (marketKey == 1)
                    {
                        ...
                        {
                            //maxShadowPrice = dictMaxprice[constName];
        ...
        public List<Constraint> GetMaxPrice(int marketkey)
        {
            ...
            try
            {
                using (VayuConnection = new VayuDBConnection().GetInstance().GetSqlConnection())
                {
                    ...
                    if (marketkey == 1)
                    {
                        mGetMaxShadowPriceCommand.CommandText = " Select distinct c.ConstraintText,c.ContingencyText ,MAX(c.ShadowPrice) MaxShadowprice  from NewTrading.PJM.[ConstraintRT] c " +
                        // " where c.MarketDateTime >='" + startDate + "' and MarketDateTime <'" + EndDate + "' " +
                        " group by c.ContingencyText ,c.ConstraintText";
                    }
                    else if (marketkey == 9)
                    {
                        mGetMaxShadowPriceCommand.CommandText = " select distinct c.ConstraintText, c.ContingencyText, MAX(MaxShadowPrice) MaxShadowPrice, MAX(ShadowPrice) MaxRTShadowPrice "
                            + " from Ercot.[dbo].[ConstraintRT] c  group by c.ConstraintText, c.ContingencyText ";
                    }
        ...
        public Dictionary<int, string> GetZones(int Marketkey)
        {
            ...
            using (VayuConnection = new VayuDBConnection().GetInstance().GetSqlConnection())
            {
                ...
                if (Marketkey == 1)
                    mSelectZoneCommand.CommandText = "Select NodeKey,Zone from NewTrading..Node where MarketKey = 1";
                else
                    mSelectZoneCommand.CommandText = "Select NodeKey,Zone from ercot..Node where MarketKey = 9";
                //mSelectZoneCommand.Connection = SigmaDbConnection;
        ...
        private void loadDBCommands()
        {
            ...
            mSelectERCOTDistinctConstraintCmd.CommandText = "select distinct constrainttext from ConstraintRT order by ConstraintText";
            mSelectERCOTDistinctConstraintCmd.Connection = VayuConnection;
            //
            mSelectERCOTDConstraintCmd = new SqlCommand();
            mSelectERCOTDConstraintCmd.CommandText = "select distinct constrainttext from ConstraintDA UNION select distinct constrainttext from ConstraintRT order by ConstraintText";
            mSelectERCOTDConstraintCmd.Connection = VayuConnection;
            //
            mSelectErcotDADistinctConstraintCmd = new SqlCommand();
            mSelectErcotDADistinctConstraintCmd.CommandText = "select distinct constrainttext from ConstraintDA order by ConstraintText";
            mSelectErcotDADistinctConstraintCmd.Connection = VayuConnection;
            //
            mSelectERCOTContingencyCmd = new SqlCommand();
            mSelectERCOTContingencyCmd.CommandText = "select distinct ContingencyText from ConstraintRT where ConstraintText = @ConstraintText order by ContingencyText";
            mSelectERCOTContingencyCmd.Connection = VayuConnection;
            //
            mSelectERCOTContingenciesCmd = new SqlCommand();
            mSelectERCOTContingenciesCmd.CommandText = " select distinct ContingencyText from ConstraintRT where ConstraintText = @ConstraintText UNION "
                                                    + " select distinct ContingencyText from ConstraintDA where ConstraintText = @ConstraintText order by ContingencyText";
            mSelectERCOTContingenciesCmd.Connection = VayuConnection;
            //
            mSelectERCOTDAContingencyCmd = new SqlCommand();
            mSelectERCOTDAContingencyCmd.CommandText = "select distinct ContingencyText from ConstraintDA where ConstraintText = @ConstraintText order by ContingencyText";
            ...
            mSelectERCOTHistoricConstraintData.CommandText = " select distinct constraintText, contingencyText, CAST(MarketDateTime as date) Date, "
                + " DATEPART(HOUR, MarketDateTime) AS Hour, ShadowPrice from ConstraintRT  where ConstraintText = @ConstraintText "
            ...
            //mSelectDAERCOTMHistoricConstraintData.CommandText = "select distinct constraintText , contingencyText, CAST(MarketDateTime as date) date , DATEPART(HOUR , MarketDateTime) , sum(abs(ShadowPrice))/12  from ERCOT..ConstraintDA where ConstraintText = @ConstraintText"
            //        + " group by constraintText , contingencyText, CAST(MarketDateTime as date) , DATEPART(HOUR , MarketDateTime)  order by date desc";//	and ContingencyText = @ContingencyText
            mSelectDAERCOTMHistoricConstraintData.CommandText = "select distinct constraintText , contingencyText, CAST(MarketDateTime as date) date , DATEPART(HOUR , MarketDateTime) , abs(ShadowPrice)  from ConstraintDA where ConstraintText = @ConstraintText order by date desc";
                    ...
                    using (SqlCommand cmd = con.CreateCommand())
                    {
                        if (marketKey == 9)
                        {
                            cmd.CommandText = isDa ? "select ConstraintName,ContingencyName,MarketDateTime,MarginalValue from MarginalValueDA(nolock)  where MarketKey=9 and MarketDateTime>=@start and MarketDateTime<@end "
                                                   : "select ConstraintText,ContingencyText,MarketDateTime,ShadowPrice from ConstraintRT(nolock) where MarketDateTime>=@start and MarketDateTime<@end ";
                        }
                        else if (marketKey == 3)
                        {
            ...
            using (SqlConnection con = new VayuDBConnection().GetInstance().GetSqlConnection())
            {
                ...
                try
                {
                    using (SqlCommand cmd = con.CreateCommand())
                    {
                        ...
                        {
                            if (isDA)
                                cmd.CommandText = "select MonitoredText , ContingencyText from DAMasterConstraint where ConstraintRTNum = " + constraintId.ToString();
                            else
                                cmd.CommandText = "select MonitoredText , ContingencyText from RTMasterConstraint where ConstraintRTNum = " + constraintId.ToString();
                        }
                        SqlDataReader rdr = cmd.ExecuteReader();
                        while (rdr.Read())
                        {
        ...
        /// <summary>
        /// Fills the constraint list.
        /// </summary>
        /// <param name="isRt">if set to <c>true</c> [is rt].</param>
        /// <returns></returns>
        /// 
        internal List<string> FillConstraintList(bool isRt, int marketId)
        {
            loadDBCommands();
        ...
        internal List<string> FillConstraintsList()
        {
            loadDBCommands();
            List<string> constraintList = new List<string>();
            if (VayuConnection.State == ConnectionState.Closed)
            ...
            rdrConstaint = mSelectERCOTDConstraintCmd.ExecuteReader();
            while (rdrConstaint.Read())
            {
                string constraint = rdrConstaint.GetValue(0).ToString();
                constraintList.Add(constraint);
        ...
        internal List<string> GetAllContingency(string constraintName)
        {
            loadDBCommands();
            List<string> contingencyList = new List<string>();
            if (VayuConnection.State == ConnectionState.Closed)
            ...
            mSelectERCOTContingenciesCmd.Parameters.AddWithValue("@ConstraintText", constraintName);
            rdrContingencies = mSelectERCOTContingenciesCmd.ExecuteReader();
            while (rdrContingencies.Read())
            {
                string contingency = rdrContingencies.GetValue(0).ToString();
                contingencyList.Add(contingency);
        ...
        /// <summary>
        /// Gets all historical constraints data.
        /// </summary>
        ...
        internal List<Constraint> GetHistoricalConstraintsForErcot(string constraintName, bool isRT, int marketKey)
        {
            ...
            if (isRT && marketKey == 9)
            {
                ...
                cmdGetConstraintHistory.CommandText = " select distinct ConstraintText, ContingencyText, MarketDateTime, ShadowPrice from ConstraintRT "
        ...
        internal Node[] RunLMP(int marketkey, DateTime StartDate, DateTime EndDate, string type)
        {
            ...
            try
            {
                ...
                ChannelFactory<ILMP> pipeFactory = new ChannelFactory<ILMP>(binding, Vayu.CommonAccessLibrary.ServiceConnections.GetLMPServiceAddress());
                ILMP nodeProxy = pipeFactory.CreateChannel();
                // int marketKey = mMarket == "MISO" ? 2 : 1;
            ...
            using (SqlConnection con = new VayuDBConnection().GetInstance().GetSqlConnection())
            {
                ...
                try
                {
                    using (SqlCommand cmd = con.CreateCommand())
                    {
                        ...
                        if (marketKey == 9)
                        {
                            cmd.CommandText = "select top 1 MarketDateTime  from ConstraintRT where  ConstraintText = @ConstraintText " +
        ...
        public Dictionary<int, string> Zones(int Marketkey)
        {
            ...
            try
            {
                using (VayuConnection = new VayuDBConnection().GetInstance().GetSqlConnection())
                {
                    ...
                    if (Marketkey == 9)
                        mSelectZoneCommand.CommandText = "Select distinct NodeKey,Zone from Node where MarketKey=9";
